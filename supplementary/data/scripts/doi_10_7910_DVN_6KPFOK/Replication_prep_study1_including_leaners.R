...
#######################
#### Read in cses and subset to relevant elections
#######################
...
# 89 is independent candidate. 90 is other party. 91 is none of the parties/candidates. 97 refused. 98 don't know. 99 missing. Assign everything from 91 to 99 and 89 should be NA
for(i in names(cses4_sub.list)){
...
#######################
#### Read in cses 3 and subset to relevant elections
#######################
...
#######################
#### Read in cses 2 and subset to relevant elections
#######################
...
#######################
#### Read in cses 1 and subset to relevant elections
#######################
...
# 0 is not applicable. 98 is don't know or no party mentioned. 99 is not available. Assign 0, 98, and 99 NA
for(i in names(cses1_sub.list)){
...
######################
## Use party_election in supp and which_close_party_election in sub cses's to assign niche and prev_enp_nat to sub cses's:
#####################

# Create vector of variable names and add in variables.
new <- c("niche","prev_enp_nat")
...
######################
## Combine the cses's using common columns and organize variables.
#####################
...
# Assign party_election in lookup form to ase10
...
nzes99$id[which(nzes99$id==0)] <- nzes99$sclose[which(nzes99$id==0)]
nzes99$id[which(nzes99$sptisan==0 & nzes99$sclose==2)] <- 3
...
nzes05$id[is.na(nzes05$id) & !is.na(nzes05$ypclose)] <- as.character(nzes05$ypclose[is.na(nzes05$id) & !is.na(nzes05$ypclose)])
nzes05$id[nzes05$id=="No"] <- NA
...
anes92$id[anes92$V923633=="5. CLOSER TO DEMOCRATIC"] <- "5"
anes92$id[anes92$V923633=="1. CLOSER TO REPUBLICAN"] <- "1"
...
######################
## Use party_election in supp and which_id_party_election in the election studies to assign niche and prev_enp_nat to the election studies:
#####################
...
######################
## Organize individual-level factors
#####################
...
#######################
####### Combine non-cses data with cses data
###########################
...
study1_incl_leaners <- dat
save(study1_incl_leaners, file="Study1_including_leaners.RData")
