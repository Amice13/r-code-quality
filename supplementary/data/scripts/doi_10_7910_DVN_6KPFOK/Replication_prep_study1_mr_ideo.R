...
for(i in 1:length(corp_cleaned)){
  ...
  prop_moral_qs_left <- c(prop_moral_qs_left, sum(counts!=0)/length(counts))
}

# Right dictionary
liwcdict <- dictionary(file = "mfd_right_wo_general.dic", format = "LIWC")
prop_moral_qs_right <- NULL
for(i in 1:length(corp_cleaned)){
  texts_split <- corpus(corp_cleaned[[i]]$content$text)
  texts_split <- tokens(texts_split)	
  temp <- dfm(texts_split, verbose = FALSE, tolower=FALSE)
  ...
  prop_moral_qs_right <- c(prop_moral_qs_right, sum(counts!=0)/length(counts))
}

# Combine the proportions with corresponding party
temp <- data.frame(party_election=names(corp_cleaned), prop_moral_qs_left = prop_moral_qs_left, prop_moral_qs_right = prop_moral_qs_right)
...
write.csv(data_collection, file="Data_mr_ideo.csv",row.names=F)
