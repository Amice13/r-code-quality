...
#####################
##### Reduce to English manifestos and clean up quasi-sentences
#####################
...
# Create a dataframe of all quasi-sentences
entire <- data.frame(manifesto_id=character(), text=character(), cmp_code=character())
for(i in 1:length(corp_cleaned)){
  temp <- data.frame(manifesto_id=corp_cleaned[[i]]$meta$manifesto_id, corp_cleaned[[i]]$content[,1:2])
  entire <- rbind(entire, temp)
}

# Run the dictionary on each manifesto, with each document being a quasi-sentence. For loop the number of times that is equal to the number of manifestos. Create vector that indicates 1 if moral word is contained and 0 if not
moral_qs <- NULL
for(i in 1:length(corp_cleaned)){
  ...
  moral_qs <- c(moral_qs, ifelse(counts==0,0,1))
}

# Add it to entire
entire$moral <- moral_qs

# Append
data_collection <- rbind(data_collection, entire)

}

# Assign id at the very end.
data_collection <- cbind(data_collection, id=1:nrow(data_collection))

# Save.
write.table(data_collection, file="Data_quasi_sentences.txt",quote=F,row.names=F)
