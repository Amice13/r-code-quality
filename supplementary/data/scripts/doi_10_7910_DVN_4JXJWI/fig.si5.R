...
## Figure SI 5: Concealed Carry Task Results (Study 1)

source("functions.R")
load("Study1.RData")

## Number of respondents by condition
N.sup <- c(table(df1[!is.na(df1$cvd) & df1$ban == 0, ]$guncong, df1[!is.na(df1$cvd) & df1$ban == 0, ]$acc))
N.opp <- c(table(df1[!is.na(df1$cvd) & df1$ban == 1, ]$guncong, df1[!is.na(df1$cvd) & df1$ban == 1, ]$acc))

## Calculate percent correct by condition
cvd.sup <- cvd.opp <- rep(NA, 4)
for (i in 1:4) {
  cvd.sup[i] <- mean(df1[df1$guncong == c(0, 1, 0, 1)[i] & df1$acc == c(0, 0, 1, 1)[i] & df1$ban == 0, ]$cvd, na.rm = T) * 100
  cvd.opp[i] <- mean(df1[df1$guncong == c(0, 1, 0, 1)[i] & df1$acc == c(0, 0, 1, 1)[i] & df1$ban == 1, ]$cvd, na.rm = T) * 100
}
