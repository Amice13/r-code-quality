...
##############################################
############# MAIN RESULTS ###################               
######## EFFECT OF BLACK NEXT DOOR ###########
####### NEIGHBOR ON WHITE PARTISANSHIP #######      
##############################################
##############################################

## This script outputs the main effect plots in Figure 2, and
## the regression tables in Tables 1, 2, 3, and 4.
...
### INDICATOR FOR NEIGHBOR WITH HIGH SCHOOL EDUCATION
...
## THERE IS A CLUSTER OF LINKED VOTERS WITH VOTERFILE AGE OVER 100 THAT ARE LIKELY MIS-RECORDED.
...
writeLines(capture.output(stargazer(mod_dem1_nocovar,mod_dem1,
                                    mod_dem_st_nocovar,mod_dem_st,
                                    mod_dem_c_nocovar,mod_dem_c,
                                    mod_dem_ed_nocovar,mod_dem_ed,
                                    style='apsr', 
                                    covariate.labels =c("Age", "High School", 'Income', 'Mover', 'HoH Age', 'Homeowner','Family Size','Employed', 'Hours per Week','Weeks per Year', paste0("Black neighbor (k=", 1:10,")"), paste0("High school educated neighbor (k=", 1:10,")")),
                                    add.lines = "FE")), "03-output/02-tables/Tab1.tex")

## TABLE 1b 


writeLines(capture.output(stargazer(mod_dem_r_nocovar,mod_dem_r,
                                    mod_dem_rp10_nocovar,mod_dem_rp10,
                                    mod_dem_rp5_nocovar,mod_dem_rp5,
                                    style='apsr', 
                                    covariate.labels =c("Age", "High School", 'Income', 'Mover', 'HoH Age', 'Homeowner','Family Size','Employed', 'Hours per Week','Weeks per Year', paste0("Black neighbor (k=", 1:10,")"), paste0("High school educated neighbor (k=", 1:10,")")),
                                    add.lines = "FE")), "03-output/02-tables/Tab2.tex")
...
load("02-data/seg_analysis_2017.Rdata")
  ...
  writeLines(capture.output(stargazer(mod_dem1_nocovar,mod_dem1,
                                      ...
                                      style='apsr', 
                                      covariate.labels =c("Age", "High School", 'Income', 'Mover', 'HoH Age', 'Homeowner','Family Size','Employed', 'Hours per Week','Weeks per Year', paste0("Black neighbor (k=", 1:10,")"), paste0("High school educated neighbor (k=", 1:10,")")),
                                      add.lines = "FE")), "03-output/02-tables/Tab3.tex")
  ...
  writeLines(capture.output(stargazer(mod_dem_r_nocovar,mod_dem_r,
                                      ...
                                      style='apsr', 
                                      covariate.labels =c("Age", "High School", 'Income', 'Mover', 'HoH Age', 'Homeowner','Family Size','Employed', 'Hours per Week','Weeks per Year', paste0("Black neighbor (k=", 1:10,")"), paste0("High school educated neighbor (k=", 1:10,")")),
                                      add.lines = "FE")), "03-output/02-tables/Tab4.tex")
...
p=ggplot(data = plot_data %>% filter(covars=='controls' & fe %in% c('None', 'State', 'County', 'Reel', 'District', 'Page-10', 'Page-5')), aes(x = fe, y = coef, color = as.numeric(fe),fill = as.numeric(fe))) +
  ...
  geom_point(size = 4, position=position_dodge(.5), fill = "white")  +  guides(alpha=NULL, fill=NULL)+
  theme_shom()+
...
## SAVE PLOT
ggsave(file = "03-output/01-plots/Fig2.jpeg", plot = p, width = 9, height = 5, units = 'in', dpi=600)
