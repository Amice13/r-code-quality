## THIS SCRIPT ESTIMATES THE EFFECT OF BLACK NEXT DOOR NEIGHBORS USING TREATMENT
## OPERATIONALIZED AS THE NUMBER OF NEXT DOOR HOUSEHOLDS (0-2) WITH A BLACK NEXT DOOR NEIGHBOR
## RATHER THAN A BINARY TREATMENT AS IN THE MAIN SPECIFICATION
...
#### Construct alternate treatment
#df$black_neighbors_dist_1 = df$neighbor1black_dist_1+df$neighbor2black_dist_1
...
covars = c( 
  ...
  "famsize", 'employed',  'hrswork1', 'wkswork1', 'black_neighbors_dist_1',
  paste0("opposite1_dist_", 2:10), paste0("hs_dist", 1:10))

# for models with no covariates
nocovars = c('black_neighbors_dist_1',paste0("opposite1_dist_", 2:10))
...
## MAKE PLOT DATA
plot_data = as.data.frame(matrix(c(sum_dem1$coefficients['black_neighbors_dist_1',c(1,2,4)], "controls",  "none"), ncol =5, nrow=1), stringsAsFactors = F)
colnames(plot_data) = c("coef", "se", "p-value", "covars", 
                        "fe")

plot_data = rbind(plot_data, c(sum_dem_st$coefficients['black_neighbors_dist_1', c(1,2,4)], "controls",  "state"))
plot_data = rbind(plot_data, c(sum_dem_c$coefficients['black_neighbors_dist_1', c(1,2,4)], "controls",  "county"))
plot_data = rbind(plot_data, c(sum_dem_r$coefficients['black_neighbors_dist_1', c(1,2,4)], "controls",  "reel"))
plot_data = rbind(plot_data, c(sum_dem_rp5$coefficients['black_neighbors_dist_1', c(1,2,4)], "controls",  "page-5"))
plot_data = rbind(plot_data, c(sum_dem_rp10$coefficients['black_neighbors_dist_1', c(1,2,4)], "controls",  "page-10"))
plot_data = rbind(plot_data, c(sum_dem_ed$coefficients['black_neighbors_dist_1', c(1,2,4)], "controls",  "enumdist"))

plot_data = rbind(plot_data, c(sum_dem1_nocovar$coefficients['black_neighbors_dist_1', c(1,2,4)], "no controls",  "none"))
plot_data = rbind(plot_data, c(sum_dem_st_nocovar$coefficients['black_neighbors_dist_1', c(1,2,4)], "no controls",  "state"))
plot_data = rbind(plot_data, c(sum_dem_c_nocovar$coefficients['black_neighbors_dist_1', c(1,2,4)], "no controls",  "county"))
plot_data = rbind(plot_data, c(sum_dem_r_nocovar$coefficients['black_neighbors_dist_1', c(1,2,4)], "no controls",  "reel"))
plot_data = rbind(plot_data, c(sum_dem_rp5_nocovar$coefficients['black_neighbors_dist_1', c(1,2,4)], "no controls",  "page-5"))
plot_data = rbind(plot_data, c(sum_dem_rp10_nocovar$coefficients['black_neighbors_dist_1', c(1,2,4)], "no controls",  "page-10"))
plot_data = rbind(plot_data, c(sum_dem_ed_nocovar$coefficients['black_neighbors_dist_1', c(1,2,4)], "no controls",  "enumdist"))
...
p=ggplot(data = plot_data %>% filter(covars=='controls' & fe %in% c('None', 'State', 'County', 'Reel', 'District', 'Page-10', 'Page-5')), aes(x = fe, y = coef, color = as.numeric(fe),fill = as.numeric(fe))) +
  xlab('')+
  ylab('Black Next Door Neighbors Coefficient') + geom_hline(yintercept = 0, alpha = .5, linetype = "dashed")+
  geom_errorbar(aes(ymin=coef-1.96*se, ymax=coef+1.96*se), width=0, size = 1.5, 
                position=position_dodge(.5), alpha=.75) + 
  geom_point(size = 4, position=position_dodge(.5), fill = "white")  +
...
## SAVE PLOT
ggsave(file = "03-output/01-plots/FigS20.jpeg",plot = p, width = 9, height = 5, units = 'in', dpi=600)
