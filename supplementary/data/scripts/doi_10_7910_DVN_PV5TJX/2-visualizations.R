...
# Load Data -------------------------------------------------------------------
df_open   <- read_csv(here("open-loop.csv"), col_types = "fTddddddddddd")
df_step_1 <- read_csv(here("incremental-change.csv"),    col_types = "ffTdddddddd")
df_step_2 <- read_csv(here("rapid-change.csv"), col_types = "ffTdddddddd")
...
long_names <- c(sequence = "Sequence",
                ...
                ppp_f = "Feed Pump Power (%)",
                ppp_r = "Recirculation Pump Power (%)")
...
df_open |> 
  pivot_longer(c(ppp_f, ppp_r, f_p, f_r, c_p, c_r)) |> 
  mutate(name = as_factor(name)) |> 
  filter(as.numeric(sequence) == 1) |> 
  ggplot(aes(hr*60, value)) +
  geom_line() +
  facet_wrap(vars(name),
             labeller = as_labeller(long_names),
             ncol = 2,
...
ggsave(here("figures", "open-loop.png"),
...
# Plot 1
df_step_1 |> 
  pivot_longer(c(f_p, f_r, p_f, p_r, c_p, c_r, t_f), names_transform = as_factor) |> 
