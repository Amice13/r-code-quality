#############################
# LDV PSPM ANALYSIS FOR EACH PERIOD
# 
# 
# Part of:
...
form.ls <- lapply(cut_yrs[-1], function(y){
  as.formula(paste(paste0("cshp_", y),
                   ...
                   paste0("cuteth", cut_yrs[which(cut_yrs == y) -1])))
})
...
saveRDS(model.ls, file = file.path(tem_res.path, "cut_model.ls.rds"))
# model.ls <- readRDS(file.path(tem_res.path, "cut_model.ls.rds"))
...
saveRDS(se.ls, file = file.path(tem_res.path, "cut_se.ls.rds"))
...
saveRDS(model.ls, file = file.path(tem_res.path, "cut_model.ls.rds"))
# se.ls <- readRDS(file.path(tem_res.path, "cut_se.ls.rds"))
