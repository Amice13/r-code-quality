...
IHME_data_country<-IHME_data_all_countries[IHME_data_all_countries$location_name=='Australia',]

IHME_data_country_20_22<-IHME_data_all_countries_20_22[IHME_data_all_countries_20_22$location_name=='Australia',]
...
BMI_all_country<-BMI_all_country[BMI_all_country$Country=='Australia',]
...
BMI_attribute<-data.frame(country='Australia', iso3='AUS', sex=c(rep('All', 19),rep('Men', 19), rep('Women', 19)),
                          year=2017:2035)
...
cases_reported<-sum(IHME_data_country$confirmed_infections [grep('2020',IHME_data_country$date)], na.rm=T)  #362741

deaths<- cases_reported*0.01#sum(IHME_data_country$deaths_reported_mean [grep('2020',IHME_data_country$date)], na.rm=T)#6223
...
hospital<- cases_reported*median(hospital_2020/cases_2020,  na.rm=T) #cases_reported*0.1

dig_no_obs<-1#odd_ratio[1,2]
...
# if 0.1 was set for group obs_0
...
men_ratio<-BMI_ratio_year[1,5:7]*0.55/(BMI_ratio_year[1,5:7]*0.55+BMI_ratio_year[2,5:7]*0.45)

men_calc<-t(t(res_2020_db)*c(0.55,as.numeric(men_ratio)))
...
cases_2021<- sum(IHME_data_country_20_22$confirmed_infections[grep('2021',IHME_data_country$date)], na.rm=T)
...
men_calc<-t(t(res_calc)*c(0.55,as.numeric(men_ratio)))
...
both_sex_calc$Prev_COVID<-(cases_2021/0.3)/pop_saudi#0.01
...
for (i_year in years){
  ...
  pop_saudi<- pop_saudi + pop_saudi*0.017 
...
other_years_db$country<-'Australia'

other_years_db$iso3<-'AUS'
...
write.csv(other_years_db,paste(saving_folders[i],'/AUS_Estimates_2020_2025_IHME_',
...
deaths_age_ratio$BMI_low_25 <-c( 0.0009, 0.0009, 0.0009, 0.0017, 0.0120, 0.0239, 0.1111, 0.2983, 0.5504)
deaths_age_ratio$BMI_25.30 <-c( 0.0009, 0.0009, 0.0009, 0.0017, 0.0120, 0.0239, 0.1111, 0.2983, 0.5504)
deaths_age_ratio$BMI_30.35 <-c( 0.0009, 0.0009, 0.0009, 0.0017, 0.0120, 0.0239, 0.1111, 0.2983, 0.5504)

new_ratio<- sum(c(0.0009, 0.0009, 0.0009, 0.0017, 0.0120, 0.0239, 0.1111, 0.2983, 0.5504))/sum(c(0,0, 0.0009, 0.0017, 0.0120, 0.0239, 0.1111, 0.2983, 0.5504))
deaths_age_ratio$BMI_above_35 <-c(0, 0, 0.0009, 0.0017, 0.0120, 0.0239, 0.1111, 0.2983, 0.5504)*new_ratio
...
write.csv(db_excess_data, paste(saving_folders[i],'/AUS_Excess_cases_hosp_deaths_2020_2025_IHME_',
                                type_obesity_prev[i],'.csv',sep=''))
...
age_above_85_fraction<-c(0.626, 0.265, 0.088, 0.018, 0.0018)


deaths_fraction<- c(0.0009, 0.0009, 0.0009, 0.0017, 0.0120, 0.0239, 0.1111, 0.2983, 0.5504*age_above_85_fraction )
...
plot(predict(model_age, newdata = data.frame(age=seq(0,100,1))), ylim=c(0,.35))
...
for (i_row in 1:dim(other_years_db_deaths)[1]){
  ...
  deaths_age<-round_preserve_sum(t(t(deaths_age_ratio[,-1])*estimated_deaths))
...
write.csv(db_excess_deaths_data_per_year_0_95, paste(saving_folders[i],'/AUS_Excess_deaths_age_group_per_year_0_95_2020_2025_IHME_',
                                                     type_obesity_prev[i],'.csv',sep=''))
}
