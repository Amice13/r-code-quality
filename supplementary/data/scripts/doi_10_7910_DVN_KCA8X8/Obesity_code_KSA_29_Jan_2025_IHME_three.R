...
IHME_data_country<-IHME_data_all_countries[IHME_data_all_countries$location_name=='Saudi Arabia',]

IHME_data_country_20_22<-IHME_data_all_countries_20_22[IHME_data_all_countries_20_22$location_name=='Saudi Arabia',]
...
BMI_all_country<-BMI_all_country[BMI_all_country$Country=='Saudi Arabia',]
...
BMI_attribute<-data.frame(country='Saudi Arabia', iso3='KSA', sex=c(rep('All', 19),rep('Men', 19), rep('Women', 19)),
                          year=2017:2035)
...
men_ratio<-BMI_ratio_year[1,5:7]*0.71/(BMI_ratio_year[1,5:7]*0.71+BMI_ratio_year[2,5:7]*0.29)

men_calc<-t(t(res_2020_db)*c(0.71,as.numeric(men_ratio)))
...
second_dose<-pop_saudi*0.92-third_dose

first_dose<-(pop_saudi*0.94-pop_saudi*0.92)
...
men_calc<-t(t(res_calc)*c(0.71,as.numeric(men_ratio)))
...
other_years_db$country<-'Saudi Arabia'

other_years_db$iso3<-' SAU'
...
write.csv(other_years_db,paste(saving_folders[i],'/KSA_Estimates_2020_2025_IHME_',
...
deaths_age_ratio$BMI_low_25 <-c(0.005,0.002,0.022,0.087,0.192,0.278, 0.212, 0.121, 0.081)
deaths_age_ratio$BMI_25.30 <-c(0.005,0.002,0.022,0.087,0.192,0.278, 0.212, 0.121, 0.081)
deaths_age_ratio$BMI_30.35 <-c(0.005,0.002,0.022,0.087,0.192,0.278, 0.212, 0.121, 0.081)

new_ratio<- sum(c(0.005,0.002,0.022,0.087,0.192,0.278, 0.212, 0.121, 0.081))/sum(c(0,0,0.022,0.087,0.192,0.278, 0.212, 0.121, 0.081))
deaths_age_ratio$BMI_above_35 <-c(0,0,0.022,0.087,0.192,0.278, 0.212, 0.121, 0.081)*new_ratio
...
write.csv(db_excess_data, paste(saving_folders[i],'/KSA_Excess_cases_hosp_deaths_2020_2025_IHME_',
                                type_obesity_prev[i],'.csv',sep=''))
...
deaths_fraction<- c(0.005,0.002,0.022,0.087,0.192,0.278, 0.212, 0.121, 0.081*age_above_85_fraction )
...
write.csv(db_excess_deaths_data_per_year_0_95, paste(saving_folders[i],'/KSA_Excess_deaths_age_group_per_year_0_95_2020_2025_IHME_',
                                                     type_obesity_prev[i],'.csv',sep=''))
