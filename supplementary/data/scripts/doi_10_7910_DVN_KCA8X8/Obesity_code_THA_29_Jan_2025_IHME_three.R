...
IHME_data_country<-IHME_data_all_countries[IHME_data_all_countries$location_name=='Thailand',]

IHME_data_country_20_22<-IHME_data_all_countries_20_22[IHME_data_all_countries_20_22$location_name=='Thailand',]
...
BMI_all_country<-BMI_all_country[BMI_all_country$Country=='Thailand',]
...
BMI_attribute<-data.frame(country='Thailand', iso3='THA', sex=c(rep('All', 19),rep('Men', 19), rep('Women', 19)),
                          year=2017:2035)
...
men_calc<-t(t(res_2020_db)*c(0.58,as.numeric(men_ratio)))
...
for (i_year in years){
  ...
  for (i_prev in covid_prev){
    ...
    men_ratio<-BMI_ratio_year[1,5:7]*0.58/(BMI_ratio_year[1,5:7]*0.71+BMI_ratio_year[2,5:7]*0.42)
...
other_years_db$country<-'Thailand'

other_years_db$iso3<-'THA'
...
write.csv(other_years_db,paste(saving_folders[i],'/THA_Estimates_2020_2025_IHME_',
...
deaths_age_ratio$BMI_low_25 <-c( 0.012, 0.010, 0.030, 0.050, 0.120, 0.310, 0.270, 0.100, 0.090)
deaths_age_ratio$BMI_25.30 <-c( 0.012, 0.010, 0.030, 0.050, 0.120, 0.310, 0.270, 0.100, 0.090)
deaths_age_ratio$BMI_30.35 <-c( 0.012, 0.010, 0.030, 0.050, 0.120, 0.310, 0.270, 0.100, 0.090)

new_ratio<- sum(c(0.012, 0.010, 0.030, 0.050, 0.120, 0.310, 0.270, 0.100, 0.090))/sum(c(0, 0, 0.030, 0.050, 0.120, 0.310, 0.270, 0.100, 0.090))
deaths_age_ratio$BMI_above_35 <-c(0, 0, 0.030, 0.050, 0.120, 0.310, 0.270, 0.100, 0.090)*new_ratio
...
write.csv(db_excess_data, paste(saving_folders[i],'/THA_Excess_cases_hosp_deaths_2020_2025_IHME_',
                                type_obesity_prev[i],'.csv',sep=''))
...
deaths_fraction<- c(0.012, 0.010, 0.030, 0.050, 0.120, 0.310, 0.270, 0.100, 0.090*age_above_85_fraction )
...
write.csv(db_excess_deaths_data_per_year_0_95, paste(saving_folders[i],'/THA_Excess_deaths_age_group_per_year_0_95_2020_2025_IHME_',
                                                     type_obesity_prev[i],'.csv',sep=''))
