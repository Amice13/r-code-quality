...
IHME_data_country<-IHME_data_all_countries[IHME_data_all_countries$location_name=='India',]

IHME_data_country_20_22<-IHME_data_all_countries_20_22[IHME_data_all_countries_20_22$location_name=='India',]
...
BMI_all_country<-BMI_all_country[BMI_all_country$Country=='India',]
...
BMI_attribute<-data.frame(country='India', iso3='IND', sex=c(rep('All', 19),rep('Men', 19), rep('Women', 19)),
                          year=2017:2035)
...
deaths<- sum(IHME_data_country$deaths_reported_mean [grep('2020',IHME_data_country$date)], na.rm=T)#6223
...
men_ratio<-BMI_ratio_year[1,5:7]*0.52/(BMI_ratio_year[1,5:7]*0.52+BMI_ratio_year[2,5:7]*0.48)

men_calc<-t(t(res_2020_db)*c(0.52,as.numeric(men_ratio)))
...
second_dose<-pop_saudi*0.64-third_dose

first_dose<-(pop_saudi*0.79-pop_saudi*0.64)
...
men_calc<-t(t(res_calc)*c(0.52,as.numeric(men_ratio)))
...
both_sex_calc$Prev_COVID<-(cases_2021/0.3)/pop_saudi
...
other_years_db$country<-'India'

other_years_db$iso3<-'IND'
...
write.csv(other_years_db,paste(saving_folders[i],'/IND_Estimates_2020_2025_IHME_',
...
deaths_age_ratio$BMI_low_25 <-c( 0.005, 0.007, 0.026, 0.061, 0.134, 0.253, 0.286, 0.170, 0.058)
deaths_age_ratio$BMI_25.30 <-c(0.005, 0.007, 0.026, 0.061, 0.134, 0.253, 0.286, 0.170, 0.058)
deaths_age_ratio$BMI_30.35 <-c(0.005, 0.007, 0.026, 0.061, 0.134, 0.253, 0.286, 0.170, 0.058)

new_ratio<- sum(c(0.005, 0.007, 0.026, 0.061, 0.134, 0.253, 0.286, 0.170, 0.058))/sum(c(0, 0, 0.026, 0.061, 0.134, 0.253, 0.286, 0.170, 0.058))
deaths_age_ratio$BMI_above_35 <-c(0, 0, 0.026, 0.061, 0.134, 0.253, 0.286, 0.170, 0.058)*new_ratio
...
write.csv(db_excess_data, paste(saving_folders[i],'/IND_Excess_cases_hosp_deaths_2020_2025_IHME_',
                                type_obesity_prev[i],'.csv',sep=''))
...
for(i_line in 1:dim(deaths_age_full)[1] ){
  ...
  excess_list_deaths[[length(excess_list_deaths)+1]]<- round(excess_values)
}
...
deaths_fraction<- c(0.005, 0.007, 0.026, 0.061, 0.134, 0.253, 0.286, 0.170, 0.0581*age_above_85_fraction )
...
write.csv(db_excess_deaths_data_per_year_0_95, paste(saving_folders[i],'/IND_Excess_deaths_age_group_per_year_0_95_2020_2025_IHME_',
                                                     type_obesity_prev[i],'.csv',sep=''))
