logit_correction_polyint_cor <- function(starts3, dat, otherdat, alts) {
    #' Full information model with Dahl's correction function
    #'
    #' This only returns the correction function (for plotting)
    #'
    ...
    empcatches <- Xvar[,((alts*gridnum)+1): dim(Xvar)[2]] %*% 
        gridcoef[((alts*gridnum)+1):dim(Xvar)[2]]
    ...
    return(list(LL = ld, catchLL = -sum(ldcatch), choiceLL = -sum(ldchoice)))
