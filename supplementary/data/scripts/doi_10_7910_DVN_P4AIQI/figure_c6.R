...
# remove respondents that made a swapping error (i.e., choosing a different candidate in the sixth task than in the first task although both task were identical)

dat = dat[dat$swapping_error==0,]
...
ggplot(data=mod_cdu_morerestrict_ests, aes(x=voted_for, y=coef)) + 
  ...
  ylab("Effect on Pr(choosing CDU/CSU candidate)") + xlab("Voter subsample") +
  theme_bw() +
...
ggsave("figures/figure_c6.pdf", width=7, height=5)