...
# The effect of the deportations attribute on candidate evaluation, by party of candidate

green_deport_mod = lm(eval.Gruene~deport.Gruene, data=dat) # OLS regression
...
spd_deport_mod = lm(eval.SPD~deport.SPD, data=dat)
spd_deport_mod_cluster <- coeftest(spd_deport_mod, cluster.vcov(spd_deport_mod, ~ID))

cdu_deport_mod = lm(eval.CDU~deport.CDU, data=dat)
cdu_deport_mod_cluster <- coeftest(cdu_deport_mod, cluster.vcov(cdu_deport_mod, ~ID))

afd_deport_mod = lm(eval.AfD~deport.AfD, data=dat)
afd_deport_mod_cluster <- coeftest(afd_deport_mod, cluster.vcov(afd_deport_mod, ~ID))

# The effect of the deportations attribute on candidate evaluation, by party of candidate and within the group of voters of the candidate's party

green_deport_own_mod = lm(eval.Gruene~deport.Gruene, data=dat[dat$voted_last=="Gruene",])
green_deport_own_mod_cluster <- coeftest(green_deport_own_mod, cluster.vcov(green_deport_own_mod, ~ID))

spd_deport_own_mod = lm(eval.SPD~deport.SPD, data=dat[dat$voted_last=="SPD",])
spd_deport_own_mod_cluster <- coeftest(spd_deport_own_mod, cluster.vcov(spd_deport_own_mod, ~ID))

cdu_deport_own_mod = lm(eval.CDU~deport.CDU, data=dat[dat$voted_last=="CDU",])
cdu_deport_own_mod_cluster <- coeftest(cdu_deport_own_mod, cluster.vcov(cdu_deport_own_mod, ~ID))

afd_deport_own_mod = lm(eval.AfD~deport.AfD, data=dat[dat$voted_last=="AfD",])
...
ggplot(data=mods_party_ests, aes(x=level, y=coef)) + 
  ...
  ylab("Effect on Candidate Evaluation") + xlab("") +
  theme_bw() +
...
ggsave("figures/figure_c9.pdf", width=7, height=5)
