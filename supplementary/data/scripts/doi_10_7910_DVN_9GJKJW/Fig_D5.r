# ====
# Figure D5 Replication:
# Effect of FDI on Deepest Tariff Cut across FTAs
# input:
...
# RUN_BRM <- FALSE # set to false to use saved results
RUN_BRM <- TRUE
...
if (RUN_BRM){
  fit <-
    brms::brm(
      -log(export_tariff_cut_hs6_deepest + 1) ~ fdicount_cum_lag_bin +
        ln_wldexp +
        ln_expmean +
        ln_impmean +
        polity2_P4 + 
  ...
  summary(fit)
  saveRDS(fit, paste0(REPLICATION_DATA_DIR, "brms_fit3.RDS"))
  ...
  summary(fit.exp)
  saveRDS(fit.exp, paste0(REPLICATION_DATA_DIR, "brms_fit3_exp.RDS"))

}else{
  fit <- readRDS(paste0(REPLICATION_DATA_DIR, "brms_fit3.RDS"))
  fit.exp <- readRDS(paste0(REPLICATION_DATA_DIR, "brms_fit3_exp.RDS"))
}
...
## 2. Effect plots ------
################################################################################
pooled.draws <- spread_draws(fit, b_fdicount_cum_lag_bin) 

pooled.draws.exp <- spread_draws(fit.exp, b_fdicount_cum_lag_bin) 
...
randomint.data.summary <- data.frame(outcome = c("Host tariff",
                                                 "Partner tariff"),
                                     est = c(fixef(fit)["fdicount_cum_lag_bin", 1],
                                             fixef(fit.exp)["fdicount_cum_lag_bin", 1]),
                                     lb = c(fixef(fit)["fdicount_cum_lag_bin", 3],
                                            fixef(fit.exp)["fdicount_cum_lag_bin", 3]),
                                     ub = c(fixef(fit)["fdicount_cum_lag_bin", 4],
                                            fixef(fit.exp)["fdicount_cum_lag_bin", 4]))
...
ggsave(randomint.plot, file = paste0(REPLICATION_FIG_DIR, "FD5_brm_randomint_summary.pdf"),
       width = 8, height = 6.5)