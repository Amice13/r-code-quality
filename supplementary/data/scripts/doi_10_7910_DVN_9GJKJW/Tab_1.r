...
## function for selecting elements not in the given list
`%!in%` <- Negate(`%in%`)
...
# 0. Unit Root Test----
urtest_panel <- data.plot.panel.used %>%
  dplyr::select(product.len, iso3c, year) %>%
  tidyr::spread(iso3c, -year)

urtest_panel <- column_to_rownames(urtest_panel, var = "year")
urtest_panel$year <- NULL

# standard ADF tests # Levin et al. (2002)
urtest <- purtest(urtest_panel,
                  test = "madwu",
                  exo = "none",
                  lags = "AIC", pmax = 5)
summary(urtest)
...
writeLines(tab.appendix, paste0(REPLICATION_FIG_DIR,
                         "T1_Extensive_results_Exports.tex"))
...
writeLines(tab.appendix.html, paste0(REPLICATION_FIG_DIR,
                         "T1_Extensive_results_Exports.html"))
