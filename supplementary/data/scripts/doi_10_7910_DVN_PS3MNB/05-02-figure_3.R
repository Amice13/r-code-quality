...
# 1) Dependencies
# 2) Data Import
# 3) Data Processing Figure 3
# 4) Responsiveness Plot Figure 3
##########################################################################################
# 1) Dependencies
##########################################################################################
library(dplyr)
...
plot_resp_ii <- all_topics %>%
  ...
  mutate(cov2 = case_when(cov == "Newspaper Articles" ~ "Newspapers", cov == "Tweets by Parties" ~ "Parties", cov == "Tweets by Politicians" ~ "Politicians")) %>%
  mutate(out2 = case_when(out == "Newspaper Articles" ~ "Newspapers", out == "Tweets by Parties" ~ "Parties", out == "Tweets by Politicians" ~ "Politicians")) %>%
...
ggplot(data = plot_resp_ii) +
  ...
  facet_wrap(~ cov,  ncol = 1, nrow = 3, scales = "free_y", labeller = as_labeller(facet_names)) + 
...
ggsave("../images/figure_3.pdf", width = 10, height = 10, device = cairo_pdf)
##########################################################################################
