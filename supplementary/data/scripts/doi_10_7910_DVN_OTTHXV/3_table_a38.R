...
#here are the base models(weighted)
test_mod1 <- lm(V_12_A==4 ~ treatment, data = models_data, weights = IPW)
test_mod2 <- lm(V_12_B==4 ~ treatment, data = models_data, weights = IPW)
test_mod3 <- lm(V_12_C==4 ~ treatment, data = models_data, weights = IPW)
test_mod4 <- lm(V_12_D==4 ~ treatment, data = models_data, weights = IPW)
test_mod5 <- lm(V_12_E==4 ~ treatment, data = models_data, weights = IPW)
test_mod6 <- lm(V_12_F==4 ~ treatment, data = models_data, weights = IPW)
test_mod7 <- lm(V_12_G==4 ~ treatment, data = models_data, weights = IPW)
test_mod8 <- lm(avg_index_v12 ~ treatment, data = models_data, weights = IPW)
...
table = stargazer(table, type = 'latex', 
                  title = "The Effect of Intervention on Politicians' First-Order Views (Inverse Propensity Weighted)",
                  label = 'tab:table_1',
                  model.names = F,
                  model.numbers = T,
                  digits = 3,
...
table[-c(10, 11, 12, 18, 21, 26)] %>%
  ...
  write_latex(., note_text, './_4_outputs/tables/table_a38.tex', .8)
