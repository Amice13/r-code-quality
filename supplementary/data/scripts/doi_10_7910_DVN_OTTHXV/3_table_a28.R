...
test_mod1 <- lm(V_12_A== 4 ~ treatment*democratic_support, data = models_data)
test_mod2 <- lm(V_12_B== 4 ~ treatment*democratic_support, data = models_data)
test_mod3 <- lm(V_12_C== 4 ~ treatment*democratic_support, data = models_data)
test_mod4 <- lm(V_12_D== 4 ~ treatment*democratic_support, data = models_data)
test_mod5 <- lm(V_12_E== 4 ~ treatment*democratic_support, data = models_data)
test_mod6 <- lm(V_12_F== 4 ~ treatment*democratic_support, data = models_data)
test_mod7 <- lm(V_12_G== 4 ~ treatment*democratic_support, data = models_data)
test_mod8 <- lm(avg_index_v12 ~ treatment*democratic_support, data = models_data)
...
table = stargazer(table, type = 'latex', 
                  title = "The Effect of Informational Treatment by Support for Democracy (First-order Beliefs)",
...
table[-c(10, 11, 12, 18, 21, 24, 27, 32)] %>%
  ...
  write_latex(., note_text, './_4_outputs/tables/table_a28.tex', .8)
