...
test_mod1 <- lm(V_61_A == 4 ~ treatment*(clim_change_misperc > 1), data = models_data)
test_mod2 <- lm(V_61_B == 4 ~ treatment*(clim_change_misperc > 1), data = models_data)
test_mod3 <- lm(V_61_C == 4 ~ treatment*(clim_change_misperc > 1), data = models_data)
test_mod4 <- lm(V_61_D == 4 ~ treatment*(clim_change_misperc > 1), data = models_data)
test_mod5 <- lm(V_61_E == 4 ~ treatment*(clim_change_misperc > 1), data = models_data)
test_mod6 <- lm(V_62_A == 4 ~ treatment*(clim_change_misperc > 1), data = models_data)
test_mod7 <- lm(V_62_B == 4 ~ treatment*(clim_change_misperc > 1), data = models_data)
...
table = stargazer(table, type = 'latex', 
                  title = "The Effect of Informational Treatment by Scale of Misperception",
                  label = 'tab:effect_mispercept_dich',
                  model.names = F,
                  model.numbers = T,
                  digits = 3,
                  ...
                  covariate.labels = c("Treatment ($T$)", "High Misperception ($H-M$)", "$T$ X $H-M$"),
...
table[-c(10, 11, 12, 18, 21, 24, 27, 32)] %>%
  ...
  write_latex(., note_text, './_4_outputs/tables/table_a18.tex', .8)
