.pol_data <-
  ...
  filter(A == "Wave 1") %>%
  ungroup() %>%
  summarise_at(vars(V_13_A, V_13_B, V_13_C, V_13_D, V_13_E, V_13_F, V_13_G), funs(avg = mean(. == 4, na.rm = T),
                                                                                  sd = sd(. == 4, na.rm = T),
                                                                                  obs = n())) %>%
  ...
  mutate(label = case_when(var == "V_13_A" ~ "Health",
                           ...
                           var == "V_13_G" ~ "Economy")) %>%
  mutate(type = "Politician second-order beliefs") %>%
  filter(var != "V_13_C")
...
fig2 <-
  ...
  theme(
    ...
    axis.text.x = element_text(color = "black"),  # Ensures x-axis value labels are black
    axis.text.y = element_text(color = "black"),  # Ensures y-axis value labels are black
    axis.title.x = element_text(color = "black", margin = margin(t = 15)),  # Space between x-axis title and labels
...
ggsave("./_4_outputs/figures/figure_2.pdf", plot = fig2, width = 8, height = 4)
write_csv(.pol_data, "./_4_outputs/supplementary/figure_2_a.csv")
write_csv(.voter_data, "./_4_outputs/supplementary/figure_2_b.csv")
