#Fit models & repeat analyses but uses different priors
rm(list=ls())
library(rstanarm)
library(tidyverse)
library(bayesplot)
...
M1 <- stan_glm(n_shootings ~ 1, 
               ...
               seed = 4641, prior_intercept = normal(0,10))
print(M1)
summary(M1)
...
M2 <- stan_glmer(n_shootings ~ 1 + (1 | oricode), 
                 ...
                 data = df, 
                 family = poisson(link="log"), 
                 seed = 4641, prior_intercept = normal(0,10),  prior_covariance = decov(scale=1.5))
print(M2)
summary(M2)
...
M3 <- stan_glmer(n_shootings ~ year + blacknh_pct + hispanic_pct + less18_pct + 
                 ...
                 offset = log(population_LEMAS), 
                 data = df, QR = T, prior_intercept = normal(0,10, autoscale = T),  prior_covariance = decov(scale=1.5), prior = normal(0,3.75, autoscale = T),
                 family = poisson(link="log"), 
                 seed = 4641) 
print(M3)
summary(M3)
...
M5 <- stan_glmer(n_shootings ~ year + blacknh_pct + hispanic_pct + less18_pct + 
                 ...
                 family = poisson(link="log"), 
                 data = df, 
                 QR = T, prior_intercept = normal(0,10, autoscale = T),  prior_covariance = decov(scale=1.5), prior = normal(0,3.75, autoscale = T),
                 seed = 4641)

print(M5)
summary(M5)
save(df, M1, M2, M3, M4, M5, file = "Data/Bayesian_models_main_diff_priors.RData")
...
jpeg("Output/Model Checks/Different Priors/sim_distribution_M1.jpeg",quality=100) 
...
gtsave(l, "Output/Model Checks/Different Priors/PPCs.html")
...
gtsave(plot.dat, "Output/Model Checks/Different Priors/LOOIC.html")
...
pd <-  pd %>% gt()
gtsave(pd, "Output/Model Checks/Different Priors/RIvar.html")
...
ggsave("Output/Model Checks/Different Priors/counterfactual_Xs.jpeg", plot= p, device="jpeg", width = 4, height = 13)
...
ggsave("Output/Model Checks/Different Priors/caterpillar_plot.jpeg", plot= g, device="jpeg", width = 8, height = 4.5)
...
ggsave("Output/Model Checks/Different Priors/map.pdf", plot = map_combo, width = 8, height = 6)
...
l <- lows %>% gt()
h <- highs %>% gt()
gtsave(l, "Output/Model Checks/Different Priors/RI_table_low.html")
gtsave(h, "Output/Model Checks/Different Priors/RI_table_high.html")
...
gtsave(pd, "Output/Model Checks/Different Priors/hypotheticals.html")
