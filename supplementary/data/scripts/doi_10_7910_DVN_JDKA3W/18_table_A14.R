...
m1 <- mdlr.conflict(dv="I(100*cw_onset_combined)",
                    x= c("lost_ga_90"),
                    cntr=c("bal_terr","tek_link","terr_maj_ag_90"),
...
m4 <- mdlr.conflict(dv="I(100*cw_onset_combined)",
                    x = c("lost_capital_only_90","lost_unity_only_90","lost_both_90"),
                    cntr=c("bal_terr","tek_link","terr_maj_ag_90",
                           "log(area_sqkm_cntr)","log(area_sqkm_ag_corr)","log(area_sqkm_corr)",
                           "terr_frac_ag_alt","terr_frac_cntr_alt",
                           "log(1+dist_capital_km)","war_hist_cw_yrs"),
...
var.labs <- c("lost_ga_90" = "Lost home rule or lost unity",
              "lost_ga_90_16" = "Lost home rule or lost unity (post-1816)",
              "lost_ga_90_pre16" = "Lost home rule or lost unity (pre-1816)",
              "lost_capital_only_90" = "Lost home rule only",
              "lost_unity_only_90" = "Lost unity only",
              "lost_both_90" = "Lost home rule & lost unity",
...
etable(m.list,tex=T, keep = c("lost","Lost"),
       dict = var.labs,
       title="Civil War Onset: National Unity Threshold 0.9",
       label="tab:cw_unity90",
       style.tex = style.tex(main="base"), 
       file=file.path(tab.path,"table_A14.tex"),replace=T,
