##### Table A19: Non-nationalist terr claims

m1 <- mdlr.conflict(dv="I(100*terr_claim_onset_no_identity)",
                    x= c("lost_capital","tek_capital_a1_b0", "lost_unity_only_any", "tek_capital_a1_b1"),
                    cntr=c("bal_terr",
                           "log(area_sqkm_cntr_b)",
                           "neighbors_ab",
                           "log(1+dist_km_ab)"),
                    fe= c("year","time_since_terr_claim_no_identity"),
                    model="ols",
...
m2 <- mdlr.conflict(dv="I(100*terr_claim_onset_no_identity)",
                    x= c("lost_capital","tek_capital_a1_b0", "lost_unity_only_any", "tek_capital_a1_b1"),
                    cntr=c("bal_terr",
                           "log(area_sqkm_cntr_b)",
                           "neighbors_ab",
                           "log(1+dist_km_ab)"),
                    fe= c("id_cap_rule_corr_a","id_cap_rule_corr_b","year","time_since_terr_claim_no_identity"),
                    model="ols",
...
m3 <- mdlr.conflict(dv="I(100*terr_claim_onset_no_identity)",
                    x= c("lost_capital","tek_capital_a1_b0", "lost_unity_only_any", "tek_capital_a1_b1"),
                    cntr=c("bal_terr",
                           "warhist_terr_claim", 
                           "log(area_sqkm_cntr_b)",
...
m4 <- mdlr.conflict(dv="I(100*terr_claim_onset_no_identity)",
                    ...
                    fe= c("id_cap_rule_corr_a","id_cap_rule_corr_b","year",
                          "time_since_terr_claim_no_identity","time_in_current_border_a","time_in_current_border_b"),
                    model="ols",
                    dat=dyad.df[dyad.df$neighbors_ab%in%c(0,1) & dyad.df$year<2002,])
...
var.labs <- c("lost_capital" = "Lost home rule & lost unity",
              ...
              "year" = "Year", "time_since_terr_claim_no_identity" = "Peace year",
              "time_in_current_border_a"="Border duration A",
              "time_in_current_border_b"="Border duration B",
              "I(100*terr_claim_onset_no_identity)"="Non-Ethnic Terr. Claim Onset $\\times 100$")
...
etable(m.list,tex=T, keep = c("lost","Lost"),
       dict = var.labs,
       title="Non-Nationalist Territorial Claims",
       label="tab:tc_nonidentity",
       style.tex = style.tex(main="base"), 
       file=file.path(tab.path,"table_A19.tex"),replace=T,
       extralines = list("-^Extended controls"=rep(c("","Yes"),each=2),
                         "-^Baseline controls"=rep("Yes",4)),
       signif.code = c("***"=0.001,"**"=0.01,"*"=0.05,"+"=0.1),
       fitstat = c("n"),
       notes = c("\\parbox[t]{\\width=\\textwidth}{\\textbf{Notes:} OLS estimates of territorial claims without an identity dimension. 
