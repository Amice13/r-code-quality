...
# After
####################  
values <- apply(model.data[,c("nlights_calib_mean.l1_orig","No.troops.country","best.ged","No.troops_sp","redeploy.period","time.since.last.best.gid","time.since.last.best.country","afterUN.period.spell","duringUN.period.spell")],2,FUN=mean)
...
n.p1 <- ggplot(data.2[data.2$Var1=="2.5%"|data.2$Var1=="97.5%"|data.2$Var1=="50%",], aes(x=Var2, y=value,fill=as.factor(Var1)))+
    geom_line(aes(linetype=Var1))+scale_linetype_manual(values=c("dashed","solid" ,"dashed"))+xlab("Years")+ ylab("Mean Calibrated Nightlight in Grid")+ 
...
setwd(pathFig)
ggsave("predMatchAfterCMeanNT.pdf",n.p1,width = 9, height = 9, units = c("cm"))
ggsave("predMatchDuringCMeanNT.pdf",n.p2, width = 9, height = 9, units = c("cm"))
