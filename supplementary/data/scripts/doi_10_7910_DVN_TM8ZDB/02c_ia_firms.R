...
## Load the shapefiles
nm_sf <- sf::st_read(dsn = "../data_raw/shapefiles/IA_counties.shp", # shapefile name from MGGG --
                     quiet = TRUE) %>% 
  sf::st_transform(crs = sf::st_crs('NAD83')) %>%
  mutate(NAME = as.character(NAME10),
         COUNTY = COUNTYFP10,
         CD = CD,
...
short <- c('PRES12', 'PRES16', 'PRES08', 'VOTE12')
...
returns <- data.table::melt(nm_df, geos, ecs) %>%
  mutate(party = substr(variable, 7,7),
         election = substr(variable, 1,6)) %>%
...
## District-level data
nm16 = nm_df %>%
  select(CD, NAME, VOTE12D, VOTE12R) %>%
  pivot_longer(cols = c(VOTE12D, VOTE12R)) %>%
  mutate(party = substr(name, 7,7),
         district=CD,
         partition = "IA")
...
save(smch, file="../data_clean/sims/ia_sims.RData")
load("../data_clean/sims/ia_sims.RData")
