...
drift_ind<-ifelse(
  (is.na(data$cgv_mil) | data$cgv_mil==0 ) &
    ...
    (is.na(data$svolik_military_scale) | data$svolik_military_scale<=2) ,1,0
) 
...
save(fit, file="static_raw_k10.RData")
output <- extract(fit, permuted = TRUE)
save(output, file="static_output_k10.RData")
options(max.print=5.5E5)
sink("static_printed_k10.txt")
print(fit)
sink()
rm(list=ls())
