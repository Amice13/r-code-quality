...
fit<- stan(model_code=model, data=stan.data, iter=5000, warmup=3000, 
           seed=10101,chains=4, cores=4,pars=c("theta_raw"),include=FALSE,
           control=list(adapt_delta=.95)) 
save(fit, file="drift_raw_k2.RData")
output <- extract(fit, permuted = TRUE)
save(output, file="drift_output_k2.RData")
options(max.print=5.5E5)
sink("drift_printed_k2.txt")
print(fit)
sink()
rm(list=ls())
