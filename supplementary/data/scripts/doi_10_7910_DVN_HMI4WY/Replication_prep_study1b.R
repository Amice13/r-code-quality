...
# Turn irrelvant responses to NA. 89 is independent candidate. 90 is other party/candidates. 91 is none of the parties/candidates. 92 and above can be treated NA. Those respondents basically did not vote.
for(i in names(cses4_sub.list)){
  cses4_sub.list[[i]]$D3006_LH_PL[cses4_sub.list[[i]]$D3006_LH_PL>=92] <- NA
}
...
# Turn irrelevant responses to NA. 89 is independent candidate. 90 is other party/candidates. 91 is none of the parties/candidates. 92 and above can be treated NA. Those respondents basically did not vote.
for(i in names(cses3_sub.list)){
  cses3_sub.list[[i]]$C3023_LH_PL[cses3_sub.list[[i]]$C3023_LH_PL>=92] <- NA
}
...
# Turn irrelevant responses to NA. 94 is inconsistent response. 96 is invalid ballot. 97 is refused. 98 is don't know.
for(i in names(cses2_sub.list)){
  cses2_sub.list[[i]]$B3006_1[cses2_sub.list[[i]]$B3006_1>=94] <- NA
}
...
# gbr 2010: p52q1
unique(gbr2010$P52Q1)

# esp 2011: H.VoteWhichRecent
unique(esp2011$H.VoteWhichRecent)
...
# Get rid of observations with NA in prev_vote_choice_cdd.
...
study1b <- combined
study1b <- study1b[,c("country_election","country_name","country_year_party_two_char","prev_vote_choice_cdd","neg_share_all_other_only","loyal_vote","ideo_scaled_inverted_0to1","age","age_sq","male","total_counts","income_scaled","educ_scaled","votet1","perceived_distance","abs_rile_change","gdp","enp_votes","left_num","lastgovt","pm_party")]
save(study1b, file="Study1b.RData")
