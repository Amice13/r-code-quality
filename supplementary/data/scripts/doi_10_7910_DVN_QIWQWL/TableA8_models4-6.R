...
data.red<-data.red[municipality!="Belo Horizonte"&broker.contact.58==0,]
...
data.lim<-data.lim[municipality!="Belo Horizonte"&broker.contact.58==0,]
data.lim<-na.omit(data.lim)
ols.lim<- lm(Y~.,data=data.lim)
summary(ols.lim)
N.lim<-nrow(data.lim)

data.full<-data.frame(Y,D,X.full)
data.full<-data.full[municipality!="Belo Horizonte"&broker.contact.58==0,]
