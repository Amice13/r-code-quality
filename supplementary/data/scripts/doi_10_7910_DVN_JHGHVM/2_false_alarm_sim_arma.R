...
# assess false alarm rates of each threshold (ARMA(1, 1) correlation)
# Taylor Grimm
# January 27th, 2024
##########################################
...
test_false_alarm <- function(ar = 0, ma = 0, skewness = 'normal', samp_size = 100, threshold_est) {
...
ic_false_alarm_sim <- function(ar = 0, ma = 0, samp_size = 100, skewness = 'normal') {
  ...
  results <- replicate(5000, test_false_alarm(ar = ar, ma = ma, skewness = skewness,
...
actual_false_alarm <- function(reps = 100, ar = 0, ma = 0, samp_size = 100, skewness = 'normal') {
  replicate(reps, ic_false_alarm_sim(ar = ar, ma = ma, samp_size = samp_size, skewness = skewness))
}
...
system.time(all_false_alarm_normal <- future_pmap(all_combinations,
                                                  function(ar, ma, n) actual_false_alarm(reps = 1000, ar = ar, ma = ma, samp_size = n, skewness = 'normal'),
...
# save(all_false_alarm_normal, file = 'false_alarms/all_false_alarm_normal_arma.rds')

# false alarm rate results for "heavier" skewness
system.time(all_false_alarm_skewed <- future_pmap(all_combinations,
                                                  function(ar, ma, n) actual_false_alarm(reps = 1000, ar = ar, ma = ma, samp_size = n, skewness = 'heavier'),
                                                  .progress = T, .options=furrr_options(seed = TRUE)))
...
# save(all_false_alarm_skewed, file = 'false_alarms/all_false_alarm_skewed_arma.rds')

# false alarm rate results for "gamma" skewness
system.time(all_false_alarm_gamma <- future_pmap(all_combinations,
                                                 function(ar, ma, n) actual_false_alarm(reps = 1000, ar = ar, ma = ma, samp_size = n, skewness = 'gamma'),
                                                 .progress = T, .options=furrr_options(seed = TRUE)))
...
# save(all_false_alarm_gamma, file = 'false_alarms/all_false_alarm_gamma_arma.rds')
...
load('false_alarms/all_false_alarm_normal_arma.rds')
load('false_alarms/all_false_alarm_skewed_arma.rds')
load('false_alarms/all_false_alarm_gamma_arma.rds')
...
normal_alarm_means_reordered <- normal_alarm_means[, c(1, 2, 3, 4, 8, 9, 5, 10, 11, 6, 7)]
colnames(normal_alarm_means_reordered) <- c('phi', 'ma', 'n', 'Sample Quantile', 'SLVM', 'SCOTT', 'BOOT', 
...
normal_alarm_means_tab[,-c(1:3)] |> apply(1, closest_to_005)

# LaTeX table of F_{5, 20} error results
skewed_alarm_means <- cbind(all_combinations, all_false_alarm_skewed |> map_dfr(rowMeans))
skewed_alarm_means_reordered <- skewed_alarm_means[, c(1, 2, 3, 4, 8, 9, 5, 10, 11, 6, 7)]
colnames(skewed_alarm_means_reordered) <- c('phi', 'ma', 'n', 'Sample Quantile', 'SLVM', 'SCOTT', 'BOOT', 
                                            'ADJ-SLVM', 'ADJ-SCOTT', 'MB-BOOT', 'RB-BOOT')
skewed_alarm_means_tab <- skewed_alarm_means_reordered
skewed_alarm_means_tab |> xtable::xtable(digits = 4)
skewed_alarm_means_tab[,-c(1:3)] |> apply(1, closest_to_005)

# LaTeX table of Gamma error results
gamma_alarm_means <- cbind(all_combinations, all_false_alarm_gamma |> map_dfr(rowMeans))
gamma_alarm_means_reordered <- gamma_alarm_means[, c(1, 2, 3, 4, 8, 9, 5, 10, 11, 6, 7)]
colnames(gamma_alarm_means_reordered) <- c('phi', 'ma', 'n', 'Sample Quantile', 'SLVM', 'SCOTT', 'BOOT', 
                                           'ADJ-SLVM', 'ADJ-SCOTT', 'MB-BOOT', 'RB-BOOT')
gamma_alarm_means_tab <- gamma_alarm_means_reordered
gamma_alarm_means_tab |> xtable::xtable(digits = 4)
gamma_alarm_means_tab[,-c(1:3)] |> apply(1, closest_to_005)
...
true_false_alarms <- function(ar = 0, ma = 0, skewness = 'normal', samp_size = 100, threshold_est) {
  if(skewness %in% c('normal', 'heavier')) {
    ic_test_data <- ic_test_series(ar = ar, ma = ma, samp_size = samp_size, skewness = skewness)
  } else if (skewness == 'gamma'){
    ic_test_data <- one_series_gamma(ar = ar, ma = ma, n = 2000)
...
get_true_false_alarms <- function(combination, skewness = 'normal', quant_val = NULL) {
  ...
  replicate(1000, true_false_alarms(ar = ar, ma = ma, skewness = skewness, samp_size = samp_size, threshold_est = quant_val)) |> mean()
}

all_true_false_alarms <- function(combination = 1, skewness = 'normal', quant_val = NULL) {
  replicate(1000, get_true_false_alarms(combination = combination, skewness = skewness, quant_val = quant_val))
}
...
##########################################################################################
load('ic_arl/nominal_thresholds_normal_arma.rds')
load('ic_arl/nominal_thresholds_skewed_arma.rds')
load('ic_arl/nominal_thresholds_gamma_arma.rds')

library(furrr)
plan(multisession, workers = 5)
set.seed(24)

system.time(all_normal_nominal_false_alarms <- tibble(combination = 1:36,
              ...
              future_pmap(function(combination, nominal) all_true_false_alarms(combination = combination, skewness = 'normal',
                                                                               quant_val = nominal),
                          .progress = T, .options=furrr_options(seed = TRUE)))
save(all_normal_nominal_false_alarms, file = 'false_alarms/normal_nominal_false_alarms_arma.rds')
all_normal_nominal_false_alarms |> map_dbl(mean) |> round(4)

system.time(all_skewed_nominal_false_alarms <- tibble(combination = 1:36,
              ...
              future_pmap(function(combination, nominal) all_true_false_alarms(combination = combination, skewness = 'heavier',
                                                                               quant_val = nominal),
                          .progress = T, .options=furrr_options(seed = TRUE)))
save(all_skewed_nominal_false_alarms, file = 'false_alarms/skewed_nominal_false_alarms_arma.rds')
all_skewed_nominal_false_alarms |> map_dbl(mean) |> round(4)

system.time(all_gamma_nominal_false_alarms <- tibble(combination = 1:36,
              ...
              future_pmap(function(combination, nominal) all_true_false_alarms(combination = combination, skewness = 'gamma',
                                                                               quant_val = nominal),
                          .progress = T, .options=furrr_options(seed = TRUE)))
save(all_gamma_nominal_false_alarms, file = 'false_alarms/gamma_nominal_false_alarms_arma.rds')
all_gamma_nominal_false_alarms |> map_dbl(mean) |> round(4)


load('false_alarms/normal_nominal_false_alarms_arma.rds')
load('false_alarms/skewed_nominal_false_alarms_arma.rds')
load('false_alarms/gamma_nominal_false_alarms_arma.rds')
