...
for (state in states) {
  if (sum(df$state == state & !is.na(df$adv12.eavs) & !is.na(df$total12)) > 0) {
    ...
    if (sum(df$state == state & !is.na(df$adv12.eavs) & !is.na(df$total12)) > 1) {
      fit12.temp <- lm(total12 ~ adv12.eavs, df[df$state == state, ])
  ...
  if (sum(df$state == state & !is.na(df$adv16.eavs) & !is.na(df$total16)) > 0) {
    ...
    if (sum(df$state == state & !is.na(df$adv16.eavs) & !is.na(df$total16)) > 1) {
      fit16.temp <- lm(total16 ~ adv16.eavs, df[df$state == state, ])
...
write.csv(out.m, 'output/early-vote-model-county-coefficients.csv')
...
write_csv(scatter_out,'output/early-vote-county-unweighted-state-scatter.csv')
...
write.csv(holder, 'results/early-vote-model-county-stats-unweighted.csv')
