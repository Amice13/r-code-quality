simDatFiles <- list.files("SDI/simData", pattern=".*\\.RData", full.names=TRUE)
...
for (f in simDatFiles) {
  ...
  for(i in unique(res$RepID)){
    DT <- res[res$RepID==i, ]
    SimulationDescription <- DT[1, c(1,3,5,6,9,10,2)]
  ...
  save(Est.Results, file=paste0("SDI/analysisResults/analysis_", basename(f)), compress="gzip")
}
