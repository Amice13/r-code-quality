...
#import DHS data - this is the household recode file and will be used to calculate recent ownhouse
#the unit of analysis is households
#use other recode files for child or maternal variables
IR <- read.dta("/Users/rburrows/Dropbox/Geography PhD/Project Drawdown work/Data/SNIR8BDT/SNIR8BFL.DTA", convert.factors = FALSE)

#subset the data to include relevent variables
sub_df<-subset(IR, select=c(v001, v002, v005, v024, v745a)) #house and household variables

#rename the variables
names(sub_df)<-c("clust_no", "hh_no", "sample_wt", "region", "ownhouse")


#remove missing values
#sub_df<-subset(sub_df, sub_df$ownhouse<3)

#create binary variable of ownhouse
sub_df$ownhouse<-ifelse(sub_df$ownhouse==1|sub_df$ownhouse==2|sub_df$ownhouse==3,1,0)

#create weighting variable
sub_df$wt<-sub_df$sample_wt/1000000


#calculate the percent positive cases by cluster (number of ownhouseed kids out of all in a cluster)
ownhouse_clus<-ddply(sub_df,~clust_no,summarise,pct_ownhouse=weighted.mean(ownhouse, sample_wt))

#count the number of ownhouseed kids in a cluster (to be used in INLA stack)
join_df<-sub_df %>%
  group_by(clust_no) %>%
  summarise(ownhouse_person=sum(ownhouse),
            person=n())
...
ownhouse_clus$dhsid<-gps_dhs$DHSID
ownhouse_clus<-dplyr::select(ownhouse_clus,dhsid,everything())
ownhouse_clus<-dplyr::select(ownhouse_clus,-clust_no)

#merge the DHS data with the covariate data and add gps data
df<-merge(ownhouse_clus, dkc, by="dhsid")
...
save(df, file="/Users/rburrows/Dropbox/Geography PhD/Project Drawdown work/exports/df_ownhouse.RData")
...
load("/Users/rburrows/Dropbox/Geography PhD/Project Drawdown work/exports/df_ownhouse.RData")
...
#attach(df)
gam_model<-gam(pct_ownhouse~ s(ai) + s(evi) + s(lstd) + s(lstn) + s(nl) + s(pet) + s(popd) + s(pet) + s(prcp) +s(tt), data = df, family = gaussian())
#detach(df)
...
#export raster
writeRaster(gam_rast_binary, filename="gam_raster_ownhouse_binary", format = "GTiff")
...
y<-df$pct_ownhouse #define response variable 
...
#export raster
writeRaster(new_raster_binary, filename="las_raster_ownhouse_binary", format = "GTiff")
...
xgb_matrix <- xgb.DMatrix(data = as.matrix(df[,c("ai", "evi","lstd", "lstn", "nl", "pet", "popd", "prcp", "tt")]), label = df$pct_ownhouse)
...
#export raster
writeRaster(xgb_raster_binary_ext, filename="xgb_raster_ownhouse_binary", format = "GTiff")
...
predict_df_bin <- subset(bind, select=c(gam, s0, gbm, pct_ownhouse, LATNUM, LONGNUM, ownhouse_person, person))


#save dataset
save(predict_df_bin, file="predict_df_ownhouse_bin.RData")
...
gam_r<-raster("/Users/rburrows/Dropbox/Geography PhD/Project Drawdown work/Data/submodel_rasters/ownhouse/binary/gam_raster_ownhouse_binary.tif")
lasso_r<-raster("/Users/rburrows/Dropbox/Geography PhD/Project Drawdown work/Data/submodel_rasters/ownhouse/binary/las_raster_ownhouse_binary.tif")
gbm_r<-raster("/Users/rburrows/Dropbox/Geography PhD/Project Drawdown work/Data/submodel_rasters/ownhouse/binary/xgb_raster_ownhouse_binary.tif")

#import df with predictions and shapefile of DHS clusters
load("/Users/rburrows/Dropbox/Geography PhD/Project Drawdown work/exports/predict_df_ownhouse_bin.RData")
sen_shp<- st_read("/Users/rburrows/Dropbox/USAID RA/GIS_Boundaries/Shapefiles/sen_admbnda_adm0_1m_gov_ocha_20190426.shp")
gps_dhs<- st_read("/Users/rburrows/Dropbox/Geography PhD/Project Drawdown work/Data/sen_19_gps/SNGE8BFL.shp") #gps coordinates for DHS clusters
load("/Users/rburrows/Dropbox/Geography PhD/Project Drawdown work/exports/stack_all.RData")
...
stk <- inla.stack(data=list(y=predict_df_bin$ownhouse_person, n=predict_df_bin$person), #NOTE (CHANGE n= here to the number of households)
...
covs <- list.files('/Users/rburrows/Dropbox/Geography PhD/Project Drawdown work/Data/submodel_rasters/ownhouse/binary/', full.names = T) %>% stack()
...
#predict_df_bin$ownhouse_hh<-as.numeric(predict_df_bin$ownhouse_hh)
...
z <- as.matrix(results)
pr.ownhouse.out <- rasterFromXYZ(cbind(x, z))

#plot raster
plot(pr.ownhouse.out, col = RColorBrewer::brewer.pal(11, "YlGnBu"), main = "Women own house alone or jointly (%)")

proj4string(pr.ownhouse.out) <- CRS(st_crs(sen_shp)$proj4string)
writeRaster(pr.ownhouse.out, filename = "pred_ownhouse_0515.tif", format = "GTiff", overwrite = TRUE)
