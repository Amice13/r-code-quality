...
##
## This is the custom RRD function used for the analysis
## It essentially the same as the rdd::RDestimate() function from the rdd package
## The only difference is the addition of two-way clustered standard errors
### ### ### ###

my_rdd <- function (formula, data, subset = NULL, cutpoint = NULL, bw = NULL, 
          kernel = "triangular", se.type = "HC1", cluster = NULL, verbose = FALSE, 
          model = FALSE, frame = FALSE, cluster_tw = NULL, noSE = F) 
{
  call <- match.call()
  if (missing(data)) 
  ...
  if (!is.null(cluster_tw)) {
    ...
    robust.se.tw <- function(model, cluster_df) {
      vcov_tw <- multiwayvcov::cluster.vcov(model, cluster_df)
      rcse.se <- coeftest(model, vcov_tw)
      return(rcse.se[2, 2])
    }
  }
  ...
  o$intercept <- vector(length = length(bws), mode = "numeric")
  names(o$est) <- names(bws)
  names(o$intercept) <- names(bws)
  o$bw <- as.vector(bws)
  ...
  for (bw in bws) {
    ...
    if (type == "sharp") {
      ...
      if (!is.null(covs)) {
        data_m <- data.frame(Y, Tr, Xl, Xr, covs, w)
        form <- as.formula(paste("Y~Tr+Xl+Xr+", paste(names(covs), 
                                                      collapse = "+", sep = ""), sep = ""))
      }
      else {
        data_m <- data.frame(Y, Tr, Xl, Xr, w)
        form <- as.formula(Y ~ Tr + Xl + Xr)
      }
      mod <- lm(form, weights = w, data = subset(data_m, 
                                                 w > 0))
      if (verbose == TRUE) {
        cat("Model:\n")
        print(summary(mod))
      ...
      o$intercept[ibw] <- coef(mod)["(Intercept)"]
      
      if (is.null(cluster) & is.null(cluster_tw) & noSE == F) {
        o$se[ibw] <- coeftest(mod, vcovHC(model = mod, type = se.type))[2, 
                                                                2]
      }
      
      if (is.null(cluster) & is.null(cluster_tw) & noSE == T) {
        o$se[ibw] <- NA
      }
      if (!is.null(cluster) & is.null(cluster_tw)) {
        o$se[ibw] <- robust.se(mod, cluster[na.ok][w > 
                                                     0])
      }
      if (!is.null(cluster_tw)) {
        
        ## exception if all obs are from the same cluster
        for (cluster in cluster_tw) {
          ## check how many clusters are present
          if (length(unique(data[na.ok & w > 0, cluster])) == 1) {
            cluster_tw <- cluster_tw[-which(cluster_tw == cluster)]
          }
        }
        o$se[ibw] <- robust.se.tw(mod, cluster_df = data[na.ok & w > 0, 
                                                          cluster_tw])
      }
