################ 
#Estimate model for Table A1
m1 <- mdlr.conflict(dv="I(100*cw_onset_combined)",
                    x= c("lost_ga","capital_mean","terr_maj_ag_23_mean"),
...
m5 <- mdlr.conflict(dv="I(100*cw_onset_combined)",
                    x= c("lost_ga_16",
                         "lost_ga_pre16",
                         "capital_mean","terr_maj_ag_23_mean"),
                    cntr=c("bal_terr","tek_link","terr_maj_ag_23",
                           "log(area_sqkm_cntr)","log(area_sqkm_ag_corr)","log(area_sqkm_corr)",
                           "terr_frac_ag_alt","terr_frac_cntr_alt",
                           "log(1+dist_capital_km)","war_hist_cw_yrs"),
...
### prepare and save regression table
...
var.labs <- c("lost_ga" = "Lost home rule or lost unity",
              ...
              "capital_mean" = "Mean home rule (1100 to $t-1$)",
              "terr_maj_ag_23_mean" = "Mean unity (1100 to $t-1$)",
...
etable(m.list,tex=T, keep = c("lost","Lost", "Mean"),
       dict = var.labs,
       title="Civil War Onset: Structural legacies?",
       label="tab:cw_structural",
       style.tex = style.tex(main="base"), 
       file=file.path(tab.path,"table_A1.tex"),replace=T,
       extralines = list("-_Baseline controls"=rep("Yes",4),
                         "-_Extended controls"=rep(c("","","Yes","Yes"),1)),
