...
m1 <- mdlr.conflict(dv="I(100*mid_onset_initiator_dyad_fatal_terr)",
                    ...
                    fe= c("year","time_since_mid_initiator_dyad_fatal_terr"),
                    model="ols",
...
m2 <- mdlr.conflict(dv="I(100*mid_onset_initiator_dyad_fatal_terr)",
                    x= c("lost_capital","tek_capital_a1_b0", "lost_unity_only_any", "tek_capital_a1_b1"),
                    cntr=c("bal_terr",
                           "log(area_sqkm_cntr_b)",
                           "neighbors_ab",
                    ...
                    fe= c("id_cap_rule_corr_a","id_cap_rule_corr_b","year","time_since_mid_initiator_dyad_fatal_terr"),
                    model="ols",
...
m3 <- mdlr.conflict(dv="I(100*mid_onset_initiator_dyad_fatal_terr)",
                    x= c("lost_capital","tek_capital_a1_b0", "lost_unity_only_any", "tek_capital_a1_b1"),
                    cntr=c("bal_terr",
                           "warhist_mid", 
                           "log(area_sqkm_cntr_b)",
...
m4 <- mdlr.conflict(dv="I(100*mid_onset_initiator_dyad_fatal_terr)",
                    x= c("lost_capital_16","lost_capital_pre16","tek_capital_a1_b0", "lost_unity_only_any_16", "lost_unity_only_any_pre16", "tek_capital_a1_b1"),
                    cntr=c("bal_terr",
                           "warhist_mid", 
                           "log(area_sqkm_cntr_b)",
                           "log(area_sqkm_ag_alt_a)",
                    ...
                    fe= c("id_cap_rule_corr_a","id_cap_rule_corr_b","year",
                          "time_since_mid_initiator_dyad_fatal_terr","time_in_current_border_a","time_in_current_border_b"),
                    model="ols",
                    dat=dyad.df[dyad.df$neighbors_ab%in%c(0,1) & dyad.df$year<2015,])
...
var.labs <- c("lost_capital" = "Lost home rule & lost unity",
              ...
              "year" = "Year", "time_since_mid_initiator_dyad_fatal_terr" = "Peace year",
              "time_in_current_border_a"="Border duration A",
              "time_in_current_border_b"="Border duration B",
              "I(100*mid_onset_initiator_dyad_fatal_terr)"="Fatal Terr. MID Onset $\\times 100$"
)
...
etable(m.list,tex=T, keep = c("lost","Lost"),
       order = c("lost","Lost"),
       dict = var.labs,
       title="Fatal & Territory-related MIDs, 1816-2014",
       label="tab:mid_fatal_terr",
       style.tex = style.tex(main="base"), 
       file=file.path(tab.path,"table_A10.tex"),replace=T,
