########### Table A21: Fatal MIDS Logit models
m1 <- mdlr.conflict(dv="I(mid_onset_initiator_dyad_fatal)",
                    x= c("lost_capital","tek_capital_a1_b0", "lost_unity_only_any", "tek_capital_a1_b1"),
                    cntr=c("bal_terr", 
                           "log(area_sqkm_cntr_b)",
                           "neighbors_ab",
                           "log(1+dist_km_ab)",
                           "time_since_mid_initiator_dyad_fatal","I(time_since_mid_initiator_dyad_fatal^2)","I(time_since_mid_initiator_dyad_fatal^3)",
...
m2 <- mdlr.conflict(dv="I(mid_onset_initiator_dyad_fatal)",
                    x= c("lost_capital","tek_capital_a1_b0", "lost_unity_only_any", "tek_capital_a1_b1"),
                    cntr=c("bal_terr",
                           "log(area_sqkm_cntr_b)",
                           "neighbors_ab",
...
m3 <- mdlr.conflict(dv="I(mid_onset_initiator_dyad_fatal)",
                    x= c("lost_capital","tek_capital_a1_b0", "lost_unity_only_any", "tek_capital_a1_b1"),
                    cntr=c("bal_terr",
                           "warhist_mid", 
                           "log(area_sqkm_cntr_b)",
...
m4 <- mdlr.conflict(dv="I(mid_onset_initiator_dyad_fatal)",
...
var.labs <- c("lost_capital" = "Lost home rule & lost unity",
              ...
              "I(mid_onset_initiator_dyad_fatal)"="Fatal MID onset"
)
...
etable(m.list,tex=T, keep = c("lost","Lost"),
       order = c("lost","Lost"),
       dict = var.labs,
       title="Fatal MID Initiation: Logit Models",
       label="tab:mid_logit",
       style.tex = style.tex(main="base"), 
       file=file.path(tab.path,"table_A21.tex"),replace=T,
       extralines = list("-^Extended controls"=rep(c("","Yes"),each=2),
                         "-^Baseline controls"=rep("Yes",4)),
       signif.code = c("***"=0.001,"**"=0.01,"*"=0.05,"+"=0.1),
       fitstat = c("n"),
       notes = c("\\parbox[t]{\\width=\\textwidth}{\\textbf{Notes:} Logit estimates of fatal MID iniation. 
