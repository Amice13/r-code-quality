m1 <- mdlr.conflict(dv="I(100*cw_onset_combined)",
                    x= c("lost_ga_50"),
                    cntr=c("bal_terr","tek_link","terr_maj_ag"),
...
m4 <- mdlr.conflict(dv="I(100*cw_onset_combined)",
                    x = c("lost_capital_only_50","lost_unity_only_50","lost_both_50"),
                    cntr=c("bal_terr","tek_link","terr_maj_ag",
                           "log(area_sqkm_cntr)","log(area_sqkm_ag_corr)","log(area_sqkm_corr)",
                           "terr_frac_ag_alt","terr_frac_cntr_alt",
                           "log(1+dist_capital_km)","war_hist_cw_yrs"),
...
var.labs <- c("lost_ga_50" = "Lost home rule or lost unity",
              "lost_ga_50_16" = "Lost home rule or lost unity (post-1816)",
              "lost_ga_50_pre16" = "Lost home rule or lost unity (pre-1816)",
              "lost_capital_only_50" = "Lost home rule only",
              "lost_unity_only_50" = "Lost unity only",
              "lost_both_50" = "Lost home rule & lost unity",
...
etable(m.list,tex=T, keep = c("lost","Lost"),
       dict = var.labs,
       title="Civil War Onset: National Unity Threshold 0.5",
       label="tab:cw_unity50",
       style.tex = style.tex(main="base"), 
       file=file.path(tab.path,"table_A13.tex"),replace=T,
