...
#### Estimate models
##########################3
m1 <- mdlr.conflict(dv="I(100*cw_terr_onset_combined)",
                    x= c("lost_ga"),
                    cntr=c("bal_terr","tek_link","terr_maj_ag_23"),
                    fe= c("year","time_since_cw_terr_combined"),
                    model="ols",
                    dat=segments.df
)
...
m2 <- mdlr.conflict(dv="I(100*cw_terr_onset_combined)",
                    x= c("lost_ga"),
                    cntr=c("bal_terr","tek_link","terr_maj_ag_23"),
                    fe= c("year","id_cap_rule_corr","time_since_cw_terr_combined"),
                    model="ols",
                    dat=segments.df
)
...
m3 <- mdlr.conflict(dv="I(100*cw_terr_onset_combined)",
                    x= c("lost_ga"),
                    cntr=c("bal_terr","tek_link","terr_maj_ag_23",
                           "log(area_sqkm_cntr)","log(area_sqkm_ag_corr)",
                           "terr_frac_ag_alt","terr_frac_cntr_alt",
                           "log(1+dist_capital_km)","war_hist_cw_terr_yrs"),
                    fe= c("year","id_cap_rule_corr","time_since_cw_terr_combined","time_in_current_border"),
                    model="ols",
                    dat=segments.df)
summary(m3,cluster=c("id_cap_rule_corr","ag_id"))

m4 <- mdlr.conflict(dv="I(100*cw_terr_onset_combined)",
...
m5 <- mdlr.conflict(dv="I(100*cw_terr_onset_combined)",
                    x= c("lost_ga_16",
                         "lost_ga_pre16"),
                    cntr=c("bal_terr","tek_link","terr_maj_ag_23",
                           "log(area_sqkm_cntr)","log(area_sqkm_ag_corr)","log(area_sqkm_corr)",
...
var.labs <- c("lost_ga" = "Lost home rule or lost unity",
              ...
              "I(100*cw_terr_onset_combined)"="Secessionist civil war onset $\\times 100$"
)
...
etable(m.list,tex=T, keep = c("lost","Lost"),
       dict = var.labs,
       title="Secessionist Civil War Onsets, 1816-2017",
       label="tab:cw_terr",
       style.tex = style.tex(main="base"), 
       file=file.path(tab.path,"table_A9.tex"),replace=T,
       extralines = list("-_Baseline controls"=rep("Yes",5),
                         "-_Extended controls"=rep(c("","Yes","Yes","Yes","Yes"),1)),
       signif.code = c("***"=0.001,"**"=0.01,"*"=0.05,"+"=0.1),
       fitstat = c("n"),
       notes = c("\\parbox[t]{\\width=\\textwidth}{\\textbf{Notes:} OLS estimates of Territorial Civil War Onsets. The unit of analysis is the ethnic segment year. 
