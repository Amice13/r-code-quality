m1 <- mdlr.conflict(dv="I(100*mid_onset_initiator_dyad_fatal)",
                    x= c("lost_capital","tek_capital_a1_b0", "lost_unity_only_any", "tek_capital_a1_b1","area_sqkm_hist_1100_log"),
                    cntr=c("bal_terr",
...
m4 <- mdlr.conflict(dv="I(100*mid_onset_initiator_dyad_fatal)",
                    x= c("lost_capital_16","lost_capital_pre16","tek_capital_a1_b0", "lost_unity_only_any_16","lost_unity_only_any_pre16", "tek_capital_a1_b1","area_sqkm_hist_1100_log"),
                    cntr=c("bal_terr",
                           "log(area_sqkm_cntr_b)",
                           "log(area_sqkm_ag_alt_a)",
                           "log(area_sqkm_ag_alt_b)",
...
var.labs <- c("lost_capital" = "Lost home rule & lost unity",
              ...
              "area_sqkm_hist_1100_log" = "Former terr. of A in B (sqkm, log)",
...
etable(m.list,tex=T, keep = c("lost","Lost","Former"),
       order = c("lost","Lost","Former"),
       dict = var.labs,
       title="Fatal MID Initiation: Non-ethnic Revisionism?",
       label="tab:mid_revisionism",
       style.tex = style.tex(main="base"), 
       file=file.path(tab.path,"table_A7.tex"),replace=T,
