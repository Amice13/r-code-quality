...
##--Rename each of the headloss variables with just "Headloss"
for(i in 1:16) {
  colnames(city_filters[[1]][[i]])[4] <- "Headloss"
}
...
server <- function(input, output) {
  ...
  output$plot1 <- renderPlot({
    ...
    plot(den_filter$datetime[input$Obs[1]:input$Obs[2]], 
         ...
         xlab = "Date (2019-2020)",
         ylab = "NTU",
         main = paste("Filter ", input$num, ", NTU 2019-2020", sep = ""),
         type = "l")
    ...
    #----Plotting Headloss
    plot(den_filter$datetime[input$Obs[1]:input$Obs[2]], 
         den_filter$Headloss[input$Obs[1]:input$Obs[2]], 
         xlab = "Date (2019-2020)",
         ylab = "Headloss",
         main = paste("Filter ", input$num, ", Headloss 2019-2020", sep = ""),
         type = "l")
    
    if(input$outliers == TRUE){
      den_sd <- sd(den_filter$Headloss, na.rm = TRUE)
      den_mean <- mean(den_filter$Headloss, na.rm = TRUE)
      
      scale    <- as.numeric(input$outLvl)
      outliers <- den_filter[which(den_filter$Headloss > (scale * den_sd + den_mean)), ]
      
      points(outliers$datetime, outliers$Headloss, col = alpha("red", 0.3), pch = 19)
