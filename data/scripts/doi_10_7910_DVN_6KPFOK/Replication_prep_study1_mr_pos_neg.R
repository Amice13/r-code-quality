...
# Set up quanteda.
liwcdict <- dictionary(file = "mfd_pos.dic", format = "LIWC")
...
write.csv(data_collection, file="Data_mr_pos.csv",row.names=F)

###################
## Set up quanteda settings
###################

liwcdict <- dictionary(file = "mfd_neg.dic", format = "LIWC")
...
data_collection <- arrange(data_collection, country_name, election_date,party_number)
write.csv(data_collection, file="Data_mr_neg.csv",row.names=F)
