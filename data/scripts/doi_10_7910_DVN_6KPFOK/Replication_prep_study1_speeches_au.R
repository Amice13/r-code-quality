...
# Read in files for 2004, 2007, 2010, or 2013. There are 8 in total.
total <- NULL
for(i in c("2004-john-howard.txt","2004-mark-latham.txt","2007-john-howard.txt","2007-kevin-rudd.txt","2010-julia-gillard.txt","2010-tony-abbott.txt","2013-kevin-rudd.txt","2013-tony-abbott.txt")){
  total[[i]] <- readtext(i)
}

#################################
# Run the mfd_left_wo_general dictionary and calculate proportion of mr sentences
#################################
...
#################################
# Run the mfd_right_wo_general dictionary and calculate proportion of mr sentences
#################################
...
#################################
# Run the mfd dictionary and calculate proportion of mr sentences
#################################
...
dat <- data.frame(party=c("2004-john-howard.txt","2004-mark-latham.txt","2007-john-howard.txt","2007-kevin-rudd.txt","2010-julia-gillard.txt","2010-tony-abbott.txt","2013-kevin-rudd.txt","2013-tony-abbott.txt"), prop_mfd_left_sents, prop_mfd_right_sents, prop_mfd_sents)

#################################
# Add party info
#################################

dat <- cbind(dat, party_name=c("Liberal/National coalition","Australian Labor Party","Liberal/National coalition","Australian Labor Party","Australian Labor Party","Liberal/National coalition","Australian Labor Party","Liberal/National coalition"))
dat$election_party <- paste(substr(dat$party, start=1, stop=4), dat$party_name, sep="_")
...
mr <- subset(mr, country_name=="Australia")
mr <- subset(mr, party_number==63320 | party_number==63620 | party_number==63810 | party_number==63621)
mr$party <- mapvalues(mr$party_number, c(63320, 63620, 63810,63621),c("Australian Labor Party","Liberal/National coalition","National","Liberal National"))
mr <- subset(mr, party=="Liberal/National coalition"|party=="Australian Labor Party")
mr$election_party <- paste(substr(mr$election_date, start=1, stop=4), mr$party, sep="_")
mr <- arrange(mr, election_party)

temp <- cbind(mr, dat)

#################################
# Read in and add information in mr_ideo.csv
#################################

mr <- read.csv("Data_mr_ideo.csv")
mr <- subset(mr, country_name=="Australia")
mr <- subset(mr, party_number==63320 | party_number==63620 | party_number==63810 | party_number==63621)
mr$party <- mapvalues(mr$party_number, c(63320, 63620, 63810,63621),c("Australian Labor Party","Liberal/National coalition","National","Liberal National"))
mr <- subset(mr, party=="Liberal/National coalition"|party=="Australian Labor Party")
...
temp <- cbind(temp,mr[,c("prop_moral_qs_left","prop_moral_qs_right")])

#################################
# Save the data
#################################

write.csv(temp[,c("election_date","party","prop_moral_qs","prop_moral_qs_left","prop_moral_qs_right","prop_mfd_sents","prop_mfd_left_sents","prop_mfd_right_sents")], file="AU_mr_in_manifesto&speech.csv",row.names = F)
