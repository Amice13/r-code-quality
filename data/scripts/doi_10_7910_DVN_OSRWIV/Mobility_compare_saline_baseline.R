...
Mobility_values$drug <- factor(Mobility_values$drug, levels = c("saline"))

# test using LMEM & ANOVA
Mobility_values.model= lmer (Session1_Mobility ~ genotype + (1|animal_ID) + (1|expt_date), data=Mobility_values, REML=TRUE)
anova(
...
sjPlot::plot_model(Mobility_values.model, title = "Mobility_values ~ genotype + (1|animal_ID) + (1|expt_date)")
sjPlot:: tab_model(Mobility_values.model, title = "Mobility_values ~ genotype + (1|animal_ID) + (1|expt_date)",
