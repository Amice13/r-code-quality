...
data <- data %>%
        ...
        mutate(MEAN_EMPLOYMENT = mean(TOTAL_RAWNUM),
               MEAN_POLICYMAKER = mean(POLICYEMPLOYEE_DC_GENRESEARCHINCLUDED_RAWNUM,na.rm = T)) %>%
        ungroup() 

data <- data %>%
        filter(MEAN_EMPLOYMENT > 15) %>%
        filter(POLICYEMPLOYEE_DC_GENRESEARCHINCLUDED_RAWNUM > 0) %>%
...
#### Model 016
items016 <- c("POLICYEMPLOYEE_DC_GENRESEARCHINCLUDED_RAWNUM_LOG",
              "POLICYEMPLOYEE_DC_GENRESEARCHINCLUDED_PROP",
              "POLICYEMPLOYEE_DC_GENRESEARCHINCLUDED_LOS_MEAN",
              "POLICYEMPLOYEE_DC_GENRESEARCHINCLUDED_SALARY_MEAN",
              "POLICYEMPLOYEE_DC_GENRESEARCHINCLUDED_POSTCOLLEGE_PROP")
name016 <- "policymaking_scores_m016"
Capacity_IRT(items016,name016)
rm(items016,name016)