...
#countries <- c("FIN","SWE") #c("USA","CAN","SGP","KOR") 
...
# Select Variables relevant for regression analysis -----------------------
  ...
  model_df <-  df %>%
    select(ends_with("mean"), ends_with("scie"), escs , mean_escs,  mig_2nd , mean_mig , langn,
...
# Check, if there are any migrants present in the analysis ----------------
...
share_migrants <-  migration_yes_no$n[2] / nrow(model_df)
...
# Generate BIFIE-Data -----------------------------------------------------
  ...
  xlsx::write.xlsx(export_data,paste0("2015_",country,".xlsx"),
                   showNA=FALSE)
  ...
  xlsx::write.xlsx(vif_dataframe,paste0(as.character(y),"_",country,"_VIF.xlsx"),
                   showNA=FALSE)
  ...
  xlsx::write.xlsx(shapiros,paste0(as.character(y),"_",country,"_sf-test.xlsx"),
                   showNA=FALSE)
