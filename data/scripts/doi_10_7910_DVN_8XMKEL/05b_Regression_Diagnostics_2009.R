...
countries <- c("DEU","GBR","USA","CAN","FIN","SWE", "SGP","KOR")
#country <- "DEU"
for(country in countries){
  print(paste("currently at:",country))
  
  
  path_year <- paste0(path,as.character(y),"/")
  setwd(path_year)
  
  
  df <- read.csv(paste0(country,"with_indices_",as.character(y),".csv")) 
  df <- as_tibble(df)
...
# Generate BIFIE-Data -----------------------------------------------------
  ...
  xlsx::write.xlsx(export_data,paste0(as.character(y),"_robustness_",country,".xlsx"),
                   showNA=FALSE)
