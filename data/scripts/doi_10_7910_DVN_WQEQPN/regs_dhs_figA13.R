...
controls.poly <- c("ruggedness_nunn_puga_mean_poly","elevation_mean_mean_poly" ,"malaria_suit_max_mean_poly","TSI_CRU_mean_1901_1920_mean_poly",
                   "temperature_fao_mean_poly",
                   "dist_coast_mean_poly_log","agric_suit_mean_poly","LONGNUM","LATNUM",
                   "dist_expl_route_mean_poly_log",
                   "dist_jedwab_city_1900_mean_poly_log",
                   "dist_capital_mean_poly_log","dist.print_log","dist.prot_log",
                   "popc_mean_1720_1890_mean_poly_log")
controls.ind <- c("age","age.sq","age.m","age.m.sq")
base.x <-  c(controls.ind, controls.poly)
...
#### #### #### #### #### #### #### #### #### #### #### #### 
#### #### Geographic Treatment: Cash crops within polygon, 1923 pubs in polygon, polygon-based controls
#### #### #### #### #### #### #### #### #### #### #### #### 

m.list <- lapply(yvars_std, function(dv){
  felm(as.formula(RegFor( y = dv , x = c("hance_crops5_sqkm_poly_mean_std","pubspc.23_poly_std",base.x) ,
...
#### #### #### #### #### #### #### #### #### #### #### #### 
#### #### Geographic Treatment: Cash crops within 15km, SOAS pubs in polygon
#### #### #### #### #### #### #### #### #### #### #### #### 

m.list <- lapply(yvars_std, function(dv){
  felm(as.formula(RegFor( y = dv , x = c("hance_crops5_sum_15km_std","pubspc.soas_poly_std", base.x) ,
                          FE = "country_survey_round" , 
                          IV="0", clust = "loc.id_pubs_poly")),
...
p2 <- p2 + geom_point(size=2.75,aes(x=beta,y=order,color=what,shape=what,fill=what)) + 
  ...
  labs(x = "Coefficients and 95% Confidence Intervals", y = "Exogamy at Ethnologue Levels",
       title="Cash Crops, Publications & Inter-Ethnic Marriages",
       subtitle="Treatment defined geographically",
       caption="Cash crop value per sqkm in 1960 USD (within 15km)\nSOAS Publications per capita (WLMS Polygon)") +
  theme_minimal(base_size = 14) +
  theme(axis.text.y = element_text(size=10),legend.position = "bottom") +
  scale_color_manual(values=c("Cash Crops" = col1, "Publications" = col2),name="Treatment") + 
  scale_fill_manual(values=c("Cash Crops" = col1, "Publications" = col2),name="Treatment") +
...
p.out <- p1 | p2
#save
p.out + ggsave(paste0(fig.path,"dhs_figA13.pdf"),width=14,height=7.5)
