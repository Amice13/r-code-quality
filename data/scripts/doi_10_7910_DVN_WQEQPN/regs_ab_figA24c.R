...
tab.path <- "results/"
fig.path <- "results/"
med.fig <- paste0(fig.path, "ab_figA24c.pdf")

#####
ab$newspaper <- as.numeric(ab$news_paper > 2)
ab$poldisc <- as.numeric(ab$politics_discuss > 1 )
ab$polint <- as.numeric(ab$politics_interest > 1 )
ab$commit <- as.numeric(ab$polact_commeet > 2)

polagg <- ab %>% subset(all.eth_poly != "") %>%
  group_by(all.eth_poly) %>%
  mutate(N=n()) %>%
  group_by(all.eth_poly, N) %>%
  summarize_at(c("poldisc", "polint", "commit") , mean, na.rm = T)
polagg$poldisc.poly <- ifelse(is.nan(polagg$poldisc), NA, polagg$poldisc)
polagg$polint.poly  <- ifelse(is.nan(polagg$polint), NA, polagg$polint)
polagg$commit.poly  <- ifelse(is.nan(polagg$commit), NA, polagg$commit)
...
"latitude", "agric_suit", "latitude", "lpopc_hyde_meanprecol_poly")
...
X <- c(controls.ind, controls.geo, "explorers_log", "dist.prot_log" , "dist.print_log")
Z <- c( "f.health", "f.water", "f.food", "urban", "cities_log", "capital_log")
...
mediators <- c("newspaper" , "poldisc", "polint", "commit", "poldisc.poly", "polint.poly", "commit.poly" )
medlabs <- c("Newspapers", "Pol. Discuss", "Pol. Interest", "Community Meet", "Av. Pol. Discuss ", "Av. Pol. Interest ", "Av. Community Meet")
...
abdf <- left_join(ab, polagg[ c("all.eth_poly", "poldisc.poly", "polint.poly", "commit.poly" )], by = "all.eth_poly")
abdf <- abdf[,c(cropvar[k],pubvar[j], yvars[i], X, Z, mediators , cl_var, "country_survey_round")]
abdf <- na.omit(abdf)
dat <- abdf
...
p <- p + geom_point() +
  ...
  labs(x = "Estimates and 95% Confidence Intervals (Cluster Bootstrap)", y = "Total and Demediated Effects",
       title="Causal Mediation Analysis - Geographic level",
