...
#### Male treatment and leavers, intensive margin only
m.list <- lapply(yvars_std, function(dv){
  ...
  data=dhs.leavers.23.m,subset=LONGNUM!=0 & iso3c%in%sample_countries_hance & stayer_wlms.m==0 &in.chr23_ethn.m!=0)
})
...
p
p + ggsave(paste0(fig.path,"dhs_figA15.pdf"),width=7,height=8)
