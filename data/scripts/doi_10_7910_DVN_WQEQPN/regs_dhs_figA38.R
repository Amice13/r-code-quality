...
#####################################################################################
######################################################################################
#### religious couples FE


couples_rel_unique <- unique(dhs[,c("v130_str","mv130_str")])
row.names(couples_rel_unique) <- NULL
couples_rel_unique <- couples_rel_unique[!is.na(couples_rel_unique$v130_str) & !is.na(couples_rel_unique$mv130_str),]
couples_rel_unique$rel_couple_directed <- paste(couples_rel_unique$v130_str,couples_rel_unique$mv130_str,sep="@")
couples_rel_unique$rel_couple_undirected <- sapply(c(1:nrow(couples_rel_unique)),function(i){
  rels <- c(couples_rel_unique$v130_str[i],couples_rel_unique$mv130_str[i])
  rels <- rels[order(rels)]
  out <- paste(rels,collapse="@")
})

dhs <- left_join(dhs,couples_rel_unique)
names(couples_rel_unique)


### geo & religion
m.list <- lapply(yvars_std, function(dv){
  felm(as.formula(RegFor( y = dv , x = c("hance_crops5_sum_15km_std","pubspc.23_poly_std", base.x) ,
                          FE = "country_survey_round + rel_couple_directed"  , 
...
##### now ethnic leavers model and ethnic treatments
...
m.list <- lapply(yvars_std, function(dv){
  felm(as.formula(RegFor( y = dv , x = c("hance_crops5_sqkm_ethn_std.m",
                                         ...
                                         "pubspc.23_ethn_std",
                                         controls.ind,
                                         "popc_mean_1720_1890_mean_ethn_log", 
                                         "popc_mean_1720_1890_mean_ethn.m_log"
                                         
                                         
  ) ,
  FE = "loc.id_pubs_poly + rel_couple_directed" , 
  IV="0", clust = "loc.id_pubs_poly")),
...
p.out <- p1 | p2
#save
p.out + ggsave(paste0(fig.path,"dhs_figA38.pdf"),width=14,height=7.5)
