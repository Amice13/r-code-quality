...
#Syntax produces:
#Figures OA4 & OA5
#The Stata syntax file 'appendixA_indivexplanation.do' needs to be run first to produce
...
m3_dydx_expl <- import("gov_satis_explain_dydx_justess_m3.dta") %>%
  dfunc1() %>%
  mutate(outcome = "Gov. Satisfaction", 
         expl = "Explanations (ESS)")

m6_dydx_expl <- import("dem_satis_explain_dydx_justess_m3.dta") %>%
  dfunc1() %>%
  mutate(outcome = "Dem. Satisfaction", 
         expl = "Explanations (ESS)")


m3_dydx_reason <- import("gov_satis_reason_dydx_justvdem_m3.dta") %>%
  dfunc1() %>%
  mutate(outcome = "Gov. Satisfaction", 
         expl = "Explanations (V-DEM")

m6_dydx_reason <- import("dem_satis_reason_dydx_justvdem_m3.dta") %>%
  dfunc1() %>%
  mutate(outcome = "Gov. Satisfaction", 
         expl = "Explanations (V-DEM")
...
m3_pred_pid_ess <- import("gov_satis_explain_pred_justess_m3.dta") %>%
  pfunc1() %>%
  mutate(outcome = "Gov. Satisfaction", 
         moderator = "Explanations (ESS)")


m6_pred_pid_ess <- import("dem_satis_explain_pred_justess_m3.dta") %>%
  pfunc1() %>%
  mutate(outcome = "Dem. Satisfaction", 
         moderator = "Explanations (ESS)")


m3_pred_pid_vdem <- import("gov_satis_reason_pred_justvdem_m3.dta") %>%
  pfunc2() %>%
  mutate(outcome = "Gov. Satisfaction", 
         moderator = "Explanations (V-DEM)")


m6_pred_pid_vdem <- import("dem_satis_reason_pred_justvdem_m3.dta") %>%
  pfunc2() %>%
  mutate(outcome = "Dem. Satisfaction", 
         moderator = "Explanations (V-DEM)")
...
##PID by ESS Explain
m3_dydx_pid_ess <- import("gov_satis_cop_explain_dydx_justess_m3.dta") %>%
  dy_func1() %>%
  mutate(outcome = "Gov. Satisfaction", 
         moderator = "Explanations (ESS)")

m6_dydx_pid_ess <- import("dem_satis_cop_explain_dydx_justess_m3.dta") %>%
  dy_func1() %>%
  mutate(outcome = "Dem. Satisfaction", 
         moderator = "Explanations (ESS)")

##PID by V-DEM Explain
m3_dydx_pid_vdem <- import("gov_satis_cop_reason_dydx_justvdem_m3.dta") %>%
  dy_func2() %>%
  mutate(outcome = "Gov. Satisfaction", 
         moderator = "Explanations (V-DEM)")

m6_dydx_pid_vdem <- import("dem_satis_cop_reason_dydx_justvdem_m3.dta") %>%
  dy_func2() %>%
  mutate(outcome = "Dem. Satisfaction", 
         moderator = "Explanations (V-DEM)")
...
##########################
#Government Satisfaction (Figure OA4)
##########################
...
ggsave("figure_oa4.png", height=14, width=12, dpi=600)

##########################
#Democratic Satisfaction (Figure OA5)
##########################
...
ggsave("figure_oa5.png", height=14, width=12, dpi=600)
