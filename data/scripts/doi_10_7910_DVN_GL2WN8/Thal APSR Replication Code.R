...
aff.lm <- lm(econcon ~ condition, data = data)
aff.pval <- pt(coef(summary(aff.lm))[, 3], summary(aff.lm)$df[2], lower=FALSE)

aff.lmg <- lm(econcon ~ condition*male, data = data)
...
plot <- summarySE(data, measurevar="econcon", 
...
aff.lm2 <- lm(econcon ~ moneycon, data = data)
...
plot <- summarySE(data[data$male == 1,], measurevar="econcon", 
									groupvars=c("condition"), 
									na.rm = T, conf.interval = .95)
...
plot <- summarySE(data[data$male == 0,], measurevar="econcon", 
									groupvars=c("condition"), 
									na.rm = T, conf.interval = .95)
