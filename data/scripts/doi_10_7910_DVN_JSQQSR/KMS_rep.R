...
non_mig <- read.csv("kms_individual_non_migrant_rep.csv")
...
#### Figure A.3: Migrant Destinations ####
...
# religion, caste, sex, age, education, employment for emigrants
mig <- mig %>%
  mutate(
    AgeAtFirstLeft = as.integer(AgeAtFirstLeft),
    male = ifelse(Sex == 1, 1, 0),
    educ = as.numeric(as.character(EducationAtMig)),
    educ = ifelse(educ==21|educ==22|educ==23, 0, educ),
    comp12st = ifelse(educ >= 12, 1, 0),
    comp10st = ifelse(educ >= 10, 1, 0), 
    age = AgeAtFirstLeft) %>%
  mutate(
    min_religion = ifelse(Hindu==1, 0, 1))


# religion, caste, sex, age, education, employment for general population
non_mig = non_mig %>% 
  mutate(
    age = Age,
    male = ifelse(Sex == 1, 1, 0),
    educ = as.numeric(as.character(EduCodeCompl)),
    educ = ifelse(educ==999, NA_integer_, educ),
    educ = ifelse(educ==21|educ==22|educ==23, 0, educ),
    comp12st = ifelse(educ >= 12, 1, 0),
    comp10st = ifelse(educ >= 10, 1, 0)) %>%
  mutate(
...
####### ANALYZE MIGRANT VS. NON-MIGRANT CHARACTERISTICS #################
library(estimatr)
library(lfe)
...
# Combine columns
means_kms <- 
  bind_cols("general_pop" = round(non_mig_means, 2), "overseas_migrants" = round(mig_means, 2))
means_kms
