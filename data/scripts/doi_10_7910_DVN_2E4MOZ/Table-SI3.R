########################################################
## This R-file produces Table3 in SI 
########################################################
...
fitPanelA2 = feols(as.formula(paste0("win_again_ofall_noind ~ id + ",nocovariates, " | master_district")), 
                   data = model24PanelA,
                   cluster = ~master_district, weights = ~wid)
fitPanelA3 = feols(win_again_ofall_noind ~ id * persig12 | master_district, data = dataPanelA,
                   cluster = ~master_district, weights = ~wid)
fitPanelA4 = feols(as.formula(paste0("win_again_ofall_noind ~ id * persig12 + ",nocovariates, " | master_district")), 
                   data = model24PanelA, 
                   cluster = ~master_district, weights = ~wid)
fitPanelA5 = feols(win_again_ofall_noind ~ id * persig12 | master_district, 
                   data = model5PanelA,
                   cluster = ~master_district, weights = ~wid)
fitPanelA6 = feols(as.formula(paste0("win_again_ofall_noind ~ id * persig12 + ",nocovariates, " | master_district")), 
                   data = model6PanelA, 
                   cluster = ~master_district, weights = ~wid)
fitPanelA7 = feols(win_again_ofall_noind ~ id * persig12 | master_district, 
                   data = model7PanelA,
                   cluster = ~master_district, weights = ~wid)
fitPanelA8 = feols(as.formula(paste0("win_again_ofall_noind ~ id * persig12 + ",nocovariates, " | master_district")), 
                   data = model8PanelA, 
                   cluster = ~master_district, weights = ~wid)
...
fitPanelB2 = feols(as.formula(paste0("win_again_noind ~ id + ",nocovariates, " | master_district")), 
                   data = model24PanelB, 
                   cluster = ~master_district, weights = ~wid)
fitPanelB3 = feols(win_again_noind ~ id * persig12 | master_district, data = dataPanelB,
                   cluster = ~master_district, weights = ~wid)
fitPanelB4 = feols(as.formula(paste0("win_again_noind ~ id * persig12 + ",nocovariates, " | master_district")), 
                   data = model24PanelB, 
                   cluster = ~master_district, weights = ~wid)
fitPanelB5 = feols(win_again_noind ~ id * persig12 | master_district, 
                   data = model5PanelB,
                   cluster = ~master_district, weights = ~wid)
fitPanelB6 = feols(as.formula(paste0("win_again_noind ~ id * persig12 + ",nocovariates, " | master_district")), 
                   data = model6PanelB, 
                   cluster = ~master_district, weights = ~wid)
fitPanelB7 = feols(win_again_noind ~ id * persig12 | master_district, 
                   data = model7PanelB,
                   cluster = ~master_district, weights = ~wid)
fitPanelB8 = feols(as.formula(paste0("win_again_noind ~ id * persig12 + ",nocovariates, " | master_district")), 
                   data = model8PanelB, 
                   cluster = ~master_district, weights = ~wid)
...
marginA3 = marginaleffects(fitPanelA3, newdata = datagrid(persig12 = dataPanelA$persig12), variables = "id")
marginA4 = marginaleffects(fitPanelA4, newdata = datagrid(persig12 = model24PanelA$persig12), variables = "id")
marginA5 = marginaleffects(fitPanelA5, newdata = datagrid(persig12 = model5PanelA$persig12), variables = "id")
marginA6 = marginaleffects(fitPanelA6, newdata = datagrid(persig12 = model6PanelA$persig12), variables = "id")
marginA7 = marginaleffects(fitPanelA7, newdata = datagrid(persig12 = model7PanelA$persig12), variables = "id")
marginA8 = marginaleffects(fitPanelA8, newdata = datagrid(persig12 = model8PanelA$persig12), variables = "id")

marginB3 = marginaleffects(fitPanelB3, newdata = datagrid(persig12 = dataPanelB$persig12), variables = "id")
marginB4 = marginaleffects(fitPanelB4, newdata = datagrid(persig12 = model24PanelB$persig12), variables = "id")
marginB5 = marginaleffects(fitPanelB5, newdata = datagrid(persig12 = model5PanelB$persig12), variables = "id")
marginB6 = marginaleffects(fitPanelB6, newdata = datagrid(persig12 = model6PanelB$persig12), variables = "id")
marginB7 = marginaleffects(fitPanelB7, newdata = datagrid(persig12 = model7PanelB$persig12), variables = "id")
marginB8 = marginaleffects(fitPanelB8, newdata = datagrid(persig12 = model8PanelB$persig12), variables = "id")
...
foreach::foreach(n=1:16)%do%{
  ...
  #for(i in 1:20){
    ...
    if(n==1){fitSim = feols(win_again_ofall_noind ~ treatment | master_district, data = dataSim,
                            cluster = ~master_district, weights = ~wid)
    }else if(n==2){fitSim = feols(as.formula(paste0("win_again_ofall_noind ~ treatment + ",nocovariates, " | master_district")), 
                                  data = dataSim,cluster = ~master_district, weights = ~wid)
    }else if(n%in%c(3,5,7)){fitSim = feols(win_again_ofall_noind ~ treatment * persig12 | master_district, data = dataSim,
                                           cluster = ~master_district, weights = ~wid)
    }else if(n%in%c(4,6,8)){fitSim = feols(as.formula(paste0("win_again_ofall_noind ~ treatment * persig12 + ",nocovariates, " | master_district")), 
                                           data = dataSim, cluster = ~master_district, weights = ~wid)
    }else if(n==9){fitSim = feols(win_again_noind ~ treatment | master_district, data = dataSim,
                                  cluster = ~master_district, weights = ~wid)
    }else if(n==10){fitSim = feols(as.formula(paste0("win_again_noind ~ treatment + ",nocovariates, " | master_district")), 
                                   data = dataSim,cluster = ~master_district, weights = ~wid)
    }else if(n%in%c(11,13,15)){fitSim = feols(win_again_noind ~ treatment * persig12 | master_district, data = dataSim,
                                              cluster = ~master_district, weights = ~wid)
    }else if(n%in%c(12,14,16)){fitSim = feols(as.formula(paste0("win_again_noind ~ treatment * persig12 + ",nocovariates, " | master_district")), 
                                              data = dataSim, cluster = ~master_district, weights = ~wid)}
...
coefficients = c("id" = "Treatment",
                    "persig12" = "Performance",
                    "id:persig12" = "Treatment x Performance")
...
rows <- tibble::tribble(~term,   ~"(1)",  ~"(2)",  ~"(3)",  ~"(4)",  ~"(5)",  ~"(6)",  ~"(7)",  ~"(8)",
                        ...
                        "ME (High Performance)",
                        "", "", as.character(round(marginA3$dydx[marginA3$persig12 == 1],3)), as.character(round(marginA4$dydx[marginA4$persig12 == 1],3)),
                        as.character(round(marginA5$dydx[marginA5$persig12 == 1],3)), as.character(round(marginA6$dydx[marginA6$persig12 == 1],3)), 
                        as.character(round(marginA7$dydx[marginA7$persig12 == 1],3)), as.character(round(marginA8$dydx[marginA8$persig12 == 1],3)),
                        "SE (High Performance)",
                        "", "", paste0("(",round(marginA3$std.error[marginA3$persig12 == 1],3),")"), paste0("(",round(marginA4$std.error[marginA4$persig12 == 1],3),")"),
                        paste0("(",round(marginA5$std.error[marginA5$persig12 == 1],3),")"), paste0("(",round(marginA6$std.error[marginA6$persig12 == 1],3),")"), 
                        paste0("(",round(marginA7$std.error[marginA7$persig12 == 1],3),")"), paste0("(",round(marginA8$std.error[marginA8$persig12 == 1],3),")"))
...
rows <- tibble::tribble(~term,   ~"(1)",  ~"(2)",  ~"(3)",  ~"(4)",  ~"(5)",  ~"(6)",  ~"(7)",  ~"(8)",
                        ...
                        "ME (High Performance)",
                        "", "", as.character(round(marginB3$dydx[marginB3$persig12 == 1],3)), as.character(round(marginB4$dydx[marginB4$persig12 == 1],3)),
                        as.character(round(marginB5$dydx[marginB5$persig12 == 1],3)), as.character(round(marginB6$dydx[marginB6$persig12 == 1],3)), 
                        as.character(round(marginB7$dydx[marginB7$persig12 == 1],3)), as.character(round(marginB8$dydx[marginB8$persig12 == 1],3)),
                        "SE (High Performance)",
                        "", "", paste0("(",round(marginB3$std.error[marginB3$persig12 == 1],3),")"), paste0("(",round(marginB4$std.error[marginB4$persig12 == 1],3),")"),
                        paste0("(",round(marginB5$std.error[marginB5$persig12 == 1],3),")"), paste0("(",round(marginB6$std.error[marginB6$persig12 == 1],3),")"), 
                        paste0("(",round(marginB7$std.error[marginB7$persig12 == 1],3),")"), paste0("(",round(marginB8$std.error[marginB8$persig12 == 1],3),")"))
