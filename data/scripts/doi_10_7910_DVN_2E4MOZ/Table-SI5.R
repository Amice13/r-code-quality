########################################################
## This R-file produces Table 5 in SI
########################################################
...
model13 = dataRaw %>% filter(!(is.na(run_again_noind_b)))
model24 = dataRaw %>% filter(!(is.na(run_again_noind_b)))  %>% 
...
model5 = dataRaw %>% filter(!(is.na(run_again_noind_b)),
                            PA3B == 0)
model6 = dataRaw %>% filter(!(is.na(run_again_noind_b)),
                            PA3B == 0) %>% 
  mutate(master_mandate_sd = scale(master_mandate),
         coun_IND_sd = scale(coun_IND),
         coun_NRM_sd = scale(coun_NRM), 
         ...
         poverty_census_ea_sd = scale(poverty_census_ea), 
         agr_share_ea_sd = scale(agr_share_ea))
model7 = dataRaw %>% filter(!(is.na(run_again_noind_b)),
                            PA3B == 2)
model8 = dataRaw %>% filter(!(is.na(run_again_noind_b)),
                            PA3B == 2) %>% 
  mutate(master_mandate_sd = scale(master_mandate),
         coun_IND_sd = scale(coun_IND),
         coun_NRM_sd = scale(coun_NRM), 
...
## Run all models
fitPanelA1 = feols(run_again_noind_b ~ id | master_district, data = model13,
                   cluster = ~master_district, weights = ~wid)
fitPanelA2 = feols(as.formula(paste0("run_again_noind_b ~ id + ",nocovariates, " | master_district")), 
                   data = model24,
                   cluster = ~master_district, weights = ~wid)
fitPanelA3 = feols(run_again_noind_b ~ id * persig14 | master_district, data = model13,
                   cluster = ~master_district, weights = ~wid)
fitPanelA4 = feols(as.formula(paste0("run_again_noind_b ~ id * persig14 + ",nocovariates, " | master_district")), 
                   data = model24, 
                   cluster = ~master_district, weights = ~wid)
fitPanelA5 = feols(run_again_noind_b ~ id * persig14 | master_district, 
                   data = model5,
                   cluster = ~master_district, weights = ~wid)
fitPanelA6 = feols(as.formula(paste0("run_again_noind_b ~ id * persig14 + ",nocovariates, " | master_district")), 
                   data = model6, 
                   cluster = ~master_district, weights = ~wid)
fitPanelA7 = feols(run_again_noind_b ~ id * persig14 | master_district, 
                   data = model7,
                   cluster = ~master_district, weights = ~wid)
fitPanelA8 = feols(as.formula(paste0("run_again_noind_b ~ id * persig14 + ",nocovariates, " | master_district")), 
                   data = model8, 
                   cluster = ~master_district, weights = ~wid)
...
foreach::foreach(n=1:8)%do%{
  ...
  #for(i in 1:20){
    ...
    if(n==1){fitSim = feols(run_again_noind_b ~ treatment | master_district, data = dataSim,
                            cluster = ~master_district, weights = ~wid)
    }else if(n==2){fitSim = feols(as.formula(paste0("run_again_noind_b ~ treatment + ",nocovariates, " | master_district")), 
                                  data = dataSim,cluster = ~master_district, weights = ~wid)
    }else if(n%in%c(3,5,7)){fitSim = feols(run_again_noind_b ~ treatment * persig14 | master_district, data = dataSim,
                                           cluster = ~master_district, weights = ~wid)
    }else if(n%in%c(4,6,8)){fitSim = feols(as.formula(paste0("run_again_noind_b ~ treatment * persig14 + ",nocovariates, " | master_district")), 
                                           data = dataSim,cluster = ~master_district, weights = ~wid)}
...
modelsummary(models, 
             ...
             gof_map = gm,
             add_rows = rows, 
             title = 'DV: Ran again (alternative PA measure).')
