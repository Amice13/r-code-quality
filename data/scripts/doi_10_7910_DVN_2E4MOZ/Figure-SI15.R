########################################################
## This R-file produces Figure 15 in SI
########################################################
...
dataHP = dataRaw %>% filter(persig14 == 1) %>% mutate(
...
fig15 = interflex(Y = "win_again_noind", D = "id", X = "PA3_11", Z = controls, 
                 data = as.data.frame(dataHP),
...
ggsave("Figure-SI15.pdf", 
       plot(fig15, main = "High Performance",
            bin.labs = FALSE, nbins = 2, theme.bw = TRUE, show.grid = FALSE,
            xlim = c(-1,1), ylim = c(-1.5, 1),
