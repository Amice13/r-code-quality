...
vars <- c("party3", "RankFree6", "RankEq6", "RankES6", "RankLO6", "RankMT6", "RankPat6")

vals <- vals[vars]

# Cut down data set to relevant vars and obs
vals <- na.omit(vals)
...
###   categories, along with value points (Fig. 5, Panel A)
###
###   
###
...
xyplot(V1 ~ V2, data = vals3,
       ...
       panel = function (x, y) {
         ...
         panel.text(mean.pty.d1[c(1,2,3)], mean.pty.d2[c(1,2,3)], 
                    labels = c("   Dem", "Ind   ", "Rep"), pos = c(3,3,2), cex = 1)
       },
       xlab = "",
       ylab = "",
...
party.mean.lengths <- c(.214, .355, .682)

party.between <- sum(party.mean.lengths * as.vector(table(vals3$party3))) - res.length

party.within <- length(vals3$V1) - sum(party.mean.lengths * as.vector(table(vals3$party3)))
