...
# Description
##########################################################################################
# Figure  A5 (lag structures)
##########################################################################################
# Contents
##########################################################################################
# 1) Dependencies
# 2) Data Import
# 3) Data Processing Figure A5
# 4) Responsiveness Plot Figure A5
##########################################################################################
# 1) Dependencies
##########################################################################################
library(dplyr)
...
plot_resp_ii <- all_topics %>%
  filter(day %in% c(1,3,5,7,9) & data_type == "Effect of a one time 10 percentage point attention increase at day 0") %>%
  mutate(cov2 = case_when(cov == "Newspaper Articles" ~ "Newspapers", 
                          cov == "Tweets by Parties" ~ "Parties", 
                          cov == "Tweets by Politicians" ~ "Politicians")) %>%
  mutate(out2 = case_when(out == "Newspaper Articles" ~ "Newspapers", 
                          out == "Tweets by Parties" ~ "Parties", 
...
# 4) Responsiveness Plot Figure A5
##########################################################################################
facet_names <- c(`Tweets by Parties` = "of Tweets by Parties with on:",
                 `Tweets by Politicians` = "of Tweets by Politicians on:",
                 `Newspaper Articles` = "of Newspaper Articles on:")

facet_names_2 <- c(`1` = "Cummulated Effect after 1 Day",
                   `3` = "Cummulated Effect after 3 Days",
                   `5` = "Cummulated Effect after 5 Days",
                   `7` = "Cummulated Effect after 7 Days",
                   `9` = "Cummulated Effect after 9 Days")
...
ggplot(data = plot_resp_ii) +
  ...
  facet_wrap(day ~ cov,  ncol = 2, nrow = 8, scales = "free_y", 
             labeller = labeller(cov = as_labeller(facet_names), 
                                 day =  as_labeller(facet_names_2))) + 
...
ggsave("../images/figure_A5.pdf", width = 12.5, height = 17.62, device = cairo_pdf)
##########################################################################################