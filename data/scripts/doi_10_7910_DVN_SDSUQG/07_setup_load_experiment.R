...
# form_full_pooled = ~ is_church + log(dist_church) + is_park +
#     is_school*children_home + log(dist_school)*children_home +
#     same_bg + same_tract * same_rr +
#     sqrt(pop/1e4) + sqrt(area) +
#     pct_race + pct_race:minority +
...
#     log(med_inc) + log(med_inc):educ_grp +
#     age + educ_grp + retired + sqrt(tenure) +
#     party + minority + homeowner + children_home + city

form_full_treat = ~ is_church + log(dist_church) + is_park +
                                is_school*children_home + log(dist_school)*children_home +
                                same_bg + same_tract * same_rr +
                                sqrt(pop/1e4) + sqrt(area) +
                                pct_race:group + pct_race:minority:group +
                                pct_party:group + pct_party:party:group +
                                pct_own:group + pct_own:homeowner:group +
                                pct_educ:group + pct_educ:educ_grp:group +
                                log(med_inc):group + log(med_inc):educ_grp:group +
                                age + educ_grp*group + retired + sqrt(tenure) +
                                party*group + minority*group + homeowner*group + children_home - group

# form_full_pooled_treat = ~ (is_church + log(dist_church) + is_park +
#     is_school*children_home + log(dist_school)*children_home +
#     same_bg + same_tract * same_rr +
#     sqrt(pop/1e4) + sqrt(area) +
#     pct_race + pct_race:minority +
...
#     log(med_inc) + log(med_inc):educ_grp +
#     age + educ_grp + retired + sqrt(tenure) +
#     party + minority + homeowner + children_home + city)*group
...
preproc = function(d) {
    ...
    mutate(block = fips,
           ...
           age_grp = case_when(age <= 40 ~ "00_40",
                               ...
                               age > 65 & age <= 75 ~ "66_75",
                               age > 75 ~ "76_99")) %>%
    select(group, city, block, block_group, tract, rr_id,
           ...
           area, any_of("ring"), dist, trust_1, trust_2, trust_3, housing) %>%
    group_by(id) %>%
