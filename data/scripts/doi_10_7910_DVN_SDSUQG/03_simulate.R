sim_paths = c(full=here("data/sim_full.rds"),
              base=here("data/sim_base.rds"),
              flip=here("data/sim_flip.rds"),
              circle=here("data/sim_circle.rds"),
              tract=here("data/sim_tract.rds"),
              zcta=here("data/sim_zcta.rds"),
              paired_full=here("data/sim_paired_full.rds"),
              paired_base=here("data/sim_paired_base.rds"))
...
if (all(file.exists(sim_paths))) {
    ...
    post_nbhds = function(resp, id, m, fit_ids, split_block_d, adj_gr, n_sim=100, pb=NULL) {
        if (!is.null(pb)) cli::cli_progress_update(id=pb)
        city_d = split_block_d[[resp$city]]
        resp_id = if (resp$id %in% fit_ids) resp$id else NULL
        nbhds = simulate_neighborhood(m[[resp$city]], resp, n_sim, resp_id,
                                      city_d, adj_gr[[resp$city]], proc_fn=preproc)
    ...
    post_circle = function(resp, id, split_block_d, adj_gr, radius=resp$radius/M_PER_MI, pb=NULL) {
        ...
        dists = as.numeric(s2::s2_distance_matrix(city_d$centroid[start_idx],
...
nbhds_d = bind_rows(resp=nbhds_resps,
                    ...
                    .id="type") %>%
    filter(id %in% unique(raw_model_d$id))
