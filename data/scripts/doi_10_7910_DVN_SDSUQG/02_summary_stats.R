...
resp_stat_path = here("paper/tables/survey-summary.tex")
...
nbhd_table_path = here("paper/tables/nbh-summary.tex")
# manual editing required for paper to tidy up labels
...
d_plot = d %>%
    ...
    mutate(city = recode(city, `miami`='Miami', `new-york`='NYC',
           ...
           name = recode(name, `pop`='Population', `sq.miles`='Area (Sq. Mi.)',
    ...
    mutate(city = factor(city, levels = c('Miami', 'NYC', 'Phoenix', 'Pooled')),
           name = factor(name, levels = c('Population', 'Area (Sq. Mi.)',
                                          'Prop. Dem.', 'Prop. Rep.',
...
p = make_sum_plot("Population", xlab=NULL) +
    ...
    make_sum_plot("Area (Sq. Mi.)") +
    make_sum_plot("Prop. Rep.") +
    make_sum_plot("Prop. Black")
if (!file.exists(figpath <- here("paper/figures/nbhd_stats_sum.pdf")) || REGENERATE) {
...
nbhd_table_path = here("paper/tables/nbh-summary-race.tex")
# manual editing required for paper to tidy up labels
...
p =
  make_sum_plot_by_race("Prop. White") +

  make_sum_plot_by_race("Prop. Black")

if (!file.exists(figpath <- here("paper/figures/nbhd_stats_sum_by_race.pdf")) || REGENERATE) {
  ggsave(figpath, p, width=6.5, height=4.5) %>%
    suppressWarnings()
}
...
nbhd_table_path = here("paper/tables/nbh-summary-party.tex")
# manual editing required for paper to tidy up labels
if (!file.exists(nbhd_table_path)) {
...
p =
  make_sum_plot_by_party("Prop. Democrat") +

  make_sum_plot_by_party("Prop. Republican")

if (!file.exists(figpath <- here("paper/figures/nbhd_stats_sum_by_party.pdf")) || REGENERATE) {
  ggsave(figpath, p, width=6.5, height=4.5) %>%
    suppressWarnings()
}
