### analysis: drop segments after first onset (aka no prior conflict)
library(dplyr)
library(here)
library(fixest)
library(ggplot2)
...
rail.df.sample <- rail.df[which(rail.df$rail_ttt_5years_binned%in%c(seq(-60,90,5),-1000,1000) &
                                  ...
                                  # only keep segments with no war history
                                  rail.df$war_hist_separatism_cw_sec_ind_aut==0),]
...
m1.twfe <- feols(separatism_cw_sec_ind_aut_yn_100 ~ rail_any_yn | seg_fe + year,
                 data=rail.df.sample,
                 cluster="seg_fe")

m1.did2s <- did2s(rail.df.sample,
                  yname = "separatism_cw_sec_ind_aut_yn_100", 
                  first_stage = ~ 0 | seg_fe + year, 
...
m2.twfe <- feols(separatism_cw_sec_ind_aut_yn_100 ~ rail_any_yn | seg_fe + country_year,
                 data=rail.df.sample,
                 cluster="seg_fe")

m2.did2s <- did2s(rail.df.sample,
                  yname = "separatism_cw_sec_ind_aut_yn_100",
                  first_stage = ~ 0 | seg_fe + country_year, 
                  second_stage = ~ rail_any_yn ,
                  treatment="rail_any_yn",
                  cluster_var = "seg_fe")
...
cap <-  "DiD Models: Dropping Cases with Past Separatism \\label{tab:did.no.past.conflict}"
note <- "\\\\scriptsize{\\\\textit{Notes:} The unit of analysis is the ethnic segment year. 
State-leading segments, segments smaller than 2000 sqkm, and those with past secessionsit civil war and claims for independence or autonomy dropped. 
...
# write out
kableExtra::save_kable(tab.out, file = here(tab.path,"table_a4.tex"))
...
p  
# ggsave(here(fig.path,"event_study_y_npc.pdf"),width=7.5,height=5.5)
...
p <- ggplot(plot.data) + 
  ...
  theme(panel.grid.minor = element_blank())
p  
# ggsave(here(fig.path,"event_study_cy_npc.pdf"),width=7.5,height=5.5)
