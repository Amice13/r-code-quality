### analysis: baseline did and event study limiting sample 1816-1922
library(dplyr)
library(here)
library(fixest)
library(ggplot2)
...
rail.df.sample <- rail.df[which(rail.df$rail_ttt_5years_binned%in%c(seq(-60,90,5),-1000,1000) &
                                  rail.df$year>1815  & rail.df$year<1923 &
...
cap <-  "Railroads and Separatism (1816-1922) \\label{tab:did.base.1922}"
...
# write out
kableExtra::save_kable(tab.out, file = here(tab.path,"table_a6.tex"))
...
es.y <- did2s(rail.df.sample,
              yname = "separatism_cw_sec_ind_aut_yn_100", 
              first_stage = ~ 0 + war_hist_terr_cw + i(time_since_cw_terr_combined) + i(time_since_ind_aut)  | seg_fe + year, 
...
plot.data$at <- c(seq(-60,-10,5),seq(0,80,5))
...
p <- ggplot(plot.data) + 
  ...
  geom_text(aes(x=13,y=5),label=lab.att,col="red")+
...
p  
ggsave(here(fig.path,"figure_a7.pdf"),width=7.5,height=5)
...
p <- ggplot(plot.data) + 
  ...
  theme(panel.grid.minor = element_blank())
p  
# ggsave(here(fig.path,"event_study_cy_1922.png"),width=7.5,height=5)
