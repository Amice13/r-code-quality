### analysis: dropping never-treated segments
library(dplyr)
library(here)
library(fixest)
library(ggplot2)
...
rail.df.sample <- rail.df[which(rail.df$rail_ttt_5years_binned %in% c(seq(-60,90,5),1000) &
                                  ...
                                  # drop never-treated
                                  rail.df$time_since_ind_aut!=130),]
...
m1.twfe <- feols(separatism_cw_sec_ind_aut_yn_100 ~ rail_any_yn +
                   war_hist_terr_cw + i(time_since_cw_terr_combined) + i(time_since_ind_aut) | seg_id + year,
                 data=rail.df.sample,
                 cluster="seg_id")

m1.did2s <- did2s(rail.df.sample[rail.df.sample$rail_ttt_5years_binned!=-1000,],
                  yname = "separatism_cw_sec_ind_aut_yn_100", 
                  first_stage = ~ 0 + war_hist_terr_cw + i(time_since_cw_terr_combined,ref=1) + 
                    i(time_since_ind_aut) | seg_id + year, 
...
m2.twfe <- feols(separatism_cw_sec_ind_aut_yn_100 ~ rail_any_yn +
                   war_hist_terr_cw + i(time_since_cw_terr_combined)  | seg_id + country_year,
                 data=rail.df.sample,
                 cluster="seg_id")

m2.did2s <- did2s(rail.df.sample,
                  yname = "separatism_cw_sec_ind_aut_yn_100",
                  first_stage = ~ 0 +  war_hist_terr_cw + i(time_since_cw_terr_combined) + 
                    i(time_since_ind_aut) | seg_id + country_year, 
                  second_stage = ~ rail_any_yn ,
                  treatment="rail_any_yn",
                  cluster_var = "seg_id")
...
rows <- tribble(~term, ~M1,  ~M2, ~M3, ~M4,
                ...
                paste(round(mean(rail.df.sample$separatism_cw_sec_ind_aut_yn_100),3)),
                paste(round(mean(rail.df.sample$separatism_cw_sec_ind_aut_yn_100[m1.did2s$obs_selection$obsRemoved]),3)),
                paste(round(mean(rail.df.sample$separatism_cw_sec_ind_aut_yn_100[m2.did2s$obs_selection$obsRemoved]),3))
)
attr(rows, 'position') <- c(3:7)
cap <-  "DiD Models: Dropping Never-Treated Units\\label{tab:did.nnt}"
...
# write out
kableExtra::save_kable(tab.out, file = here(tab.path,"table_a5.tex"))
...
es.y <- did2s(rail.df.sample,
              yname = "separatism_cw_sec_ind_aut_yn_100", 
              first_stage = ~ 0 +war_hist_terr_cw + i(time_since_cw_terr_combined) + 
                i(time_since_ind_aut,ref=1) | seg_id + year, 
...
p <- ggplot(plot.data) + 
  ...
  geom_text(aes(x=83,y=3.75),label=lab.att,col="red")+
...
p  
# ggsave(here(fig.path,"event_study_y_nnt.pdf"),width=7.5,height=5)
...
p <- ggplot(plot.data) + 
  ...
  geom_vline(xintercept=-5,color="grey40",linetype="dashed") +
  geom_point(aes(x=at,y=beta)) +
  geom_text(aes(x=83,y=8),label=lab.att,col="red")+
  scale_x_continuous(name="Time to/since first railway (5-year bins)",n.breaks=9) +
  scale_y_continuous(name="Estimate and 95% CI") +
  labs(title = "Railroad Access and Separatist Mobilization",
       subtitle = "Event Study Results")+
  ...
  theme(panel.grid.minor = element_blank())
p  
# ggsave(here(fig.path,"event_study_cy_nnt.pdf"),width=7.5,height=5)
