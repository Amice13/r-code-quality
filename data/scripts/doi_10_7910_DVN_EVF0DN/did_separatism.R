### analysis: baseline did and event study
library(dplyr)
library(here)
library(fixest)
library(ggplot2)
...
cap <-  "Railroads and Separatism (1816-1945) \\label{tab:did.base}"
...
# write out
kableExtra::save_kable(tab.out, file = here(tab.path,"table_1.tex"))
...
p <- ggplot(plot.data) + 
  ...
  geom_text(aes(x=13,y=3.75),label=lab.att,col="red")+
...
p  
ggsave(here(fig.path, "figure_3.pdf"), width=7.5, height=5)
...
p <- ggplot(plot.data) + 
  ...
  geom_vline(xintercept=-5,color="grey40",linetype="dashed") +
  geom_point(aes(x=at,y=beta)) +
  geom_text(aes(x=13,y=3.8),label=lab.att,col="red")+
  scale_x_continuous(name="Time to/since first railway (5-year bins)",n.breaks=9) +
  scale_y_continuous(name="Estimate and 95% CI") +
  labs(title = "Railroad Access and Separatist Mobilization",
       subtitle = "Event Study Results")+
  ...
  theme(panel.grid.minor = element_blank())
p  
ggsave(here(fig.path,"figure_a6.pdf"),width=7.5,height=5)
