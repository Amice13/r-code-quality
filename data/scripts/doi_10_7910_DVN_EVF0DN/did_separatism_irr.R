### analysis: Models with irredentism as outcome
library(dplyr)
library(here)
library(fixest)
library(ggplot2)
...
rail.df$separatism_cw_sec_ind_aut_irr_yn_100 <- 100*rail.df$separatism_cw_sec_ind_aut_irr_yn
...
##### Estimate Models
##### ##### ##### ##### ##### ##### ##### ##### ##### ##### 
m1.twfe <- feols(separatism_cw_sec_ind_aut_irr_yn_100 ~ rail_any_yn +
                   war_hist_terr_cw + i(time_since_cw_terr_combined) + i(time_since_ind_aut_irr) | seg_fe + year,
                 data=rail.df.sample,
                 cluster="seg_fe")

m1.did2s <- did2s(rail.df.sample,
                  yname = "separatism_cw_sec_ind_aut_irr_yn_100", 
                  first_stage = ~ 0 + war_hist_terr_cw  + i(time_since_cw_terr_combined,ref=1) + i(time_since_ind_aut_irr,ref=1)| seg_fe + year, 
...
m2.twfe <- feols(separatism_cw_sec_ind_aut_irr_yn_100 ~ rail_any_yn +
                   war_hist_terr_cw + i(time_since_cw_terr_combined) + i(time_since_ind_aut_irr)| seg_fe + country_year,
                 data=rail.df.sample,
                 cluster="seg_fe")

m2.did2s <- did2s(rail.df.sample,
                  yname = "separatism_cw_sec_ind_aut_irr_yn_100",
                  first_stage = ~ 0 +  war_hist_terr_cw +  i(time_since_cw_terr_combined) + i(time_since_ind_aut_irr) | seg_fe + country_year, 
                  second_stage = ~ rail_any_yn ,
                  treatment="rail_any_yn",
                  cluster_var = "seg_fe")
...
rows <- tribble(~term, ~M1,  ~M2, ~M3, ~M4,
                ...
                "\\midrule Mean DV",
                paste(round(mean(rail.df.sample$separatism_cw_sec_ind_aut_irr_yn_100, na.rm = T),3)),
                paste(round(mean(rail.df.sample$separatism_cw_sec_ind_aut_irr_yn_100, na.rm = T),3)),
                paste(round(mean(rail.df.sample$separatism_cw_sec_ind_aut_irr_yn_100[m1.did2s$obs_selection$obsRemoved]),3)),
                paste(round(mean(rail.df.sample$separatism_cw_sec_ind_aut_irr_yn_100[m2.did2s$obs_selection$obsRemoved]),3))
)
attr(rows, 'position') <- c(3:7)
cap <-  "Railroads and Separatism or Irredentism (1816-1945) \\label{tab:did.separatism.irr}"
...
tab.out <- modelsummary(m.list,
  ...
  add_header_above(header=c(" "=1,"100 $\\\\times$ Secession, Terr. CW or Claim (incl. Irredentism)"=4),escape=F)

# write out
kableExtra::save_kable(tab.out, file = here(tab.path,"table_a11.tex"))
...
es.y <- did2s(rail.df.sample,
              yname = "separatism_cw_sec_ind_aut_irr_yn_100", 
              first_stage = ~ 0 + war_hist_terr_cw + i(time_since_cw_terr_combined) + i(time_since_ind_aut_irr)  | seg_fe + year, 
...
names(es.y$coefficients)
wald(es.y,c("rail_ttt_5years_binned::-"),vcov = es.y$cov.scaled)
...
p <- ggplot(plot.data) + 
  ...
  theme(panel.grid.minor = element_blank())
p  
# ggsave(here(fig.path,"event_study_irr_cy.png"),width=7.5,height=5)
