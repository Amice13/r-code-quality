### analysis: nationalist claims as outcome only
...
rail.df$onset_nc_ind_aut_100 <- 100*rail.df$onset_nc_ind_aut
...
##### Estimate Models
##### ##### ##### ##### ##### ##### ##### ##### ##### ##### 
m1.twfe <- feols(onset_nc_ind_aut_100 ~ rail_any_yn  + war_hist_terr_cw + i(time_since_cw_terr_combined) + i(time_since_ind_aut)   | seg_fe + year,
                 data=rail.df.sample,
                 cluster="seg_fe")

m1.did2s <- did2s(rail.df.sample,
                  yname = "onset_nc_ind_aut_100", 
...
m2.twfe <- feols(onset_nc_ind_aut_100 ~ rail_any_yn +
...
rows <- tribble(~term, ~M1,  ~M2, ~M3, ~M4,
                ...
                "\\midrule Mean DV",
                paste(round(mean(rail.df.sample$onset_nc_ind_aut_100),3)),
                paste(round(mean(rail.df.sample$onset_nc_ind_aut_100),3)),
                paste(round(mean(rail.df.sample$onset_nc_ind_aut_100[m1.did2s$obs_selection$obsRemoved]),3)),
                paste(round(mean(rail.df.sample$onset_nc_ind_aut_100[m2.did2s$obs_selection$obsRemoved]),3))
)
attr(rows, 'position') <- c(3:7)
cap <-  "Railroads and Separatist Claims (1816-1945) \\label{tab:did.ind.aut}"
...
tab.out <- modelsummary(m.list,
  ...
  add_header_above(header=c(" "=1,"100 $\\\\times$ Independence or Autonomy Claim"=4),escape=F)

# write out
kableExtra::save_kable(tab.out, file = here(tab.path,"table_a10.tex"))
...
p <- ggplot(plot.data) + 
  ...
  labs(title = "Railroad Access and Separatist Claims",
       subtitle = "Event Study Results")+
  theme_minimal(base_size=14) +
  theme(panel.grid.minor = element_blank())
p  
ggsave(here(fig.path,"figure_a10.pdf"),width=7.5,height=5)
...
es.cy <- did2s(rail.df.sample,
               yname = "onset_nc_ind_aut_100", 
               first_stage = ~ 0 +  war_hist_terr_cw + i(time_since_cw_terr_combined,ref=1) + i(time_since_ind_aut,ref=1) | seg_fe + country_year, 
               second_stage = ~ i(rail_ttt_5years_binned, ref=c(-5,-1000,1000)),
               treatment="rail_any_yn",
               cluster_var = "seg_fe")
...
p <- ggplot(plot.data) + 
  ...
  theme(panel.grid.minor = element_blank())
p  
# ggsave(here(fig.path,"event_study_cy_ind_aut.pdf"),width=7.5,height=5)
