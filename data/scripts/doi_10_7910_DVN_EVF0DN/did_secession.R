### analysis: only successful secession as outcome
library(dplyr)
library(here)
library(fixest)
library(ggplot2)
...
rail.df$secession_yn_100 <- 100*rail.df$secession_yn
...
m1.twfe <- feols(secession_yn_100 ~ rail_any_yn + war_hist_terr_cw + i(time_since_cw_terr_combined) + 
                   i(time_since_ind_aut) | seg_fe + year,
                 data=rail.df.sample,
                 cluster="seg_fe")

m1.did2s <- did2s(rail.df.sample,
                  yname = "secession_yn_100", 
...
m2.twfe <- feols(secession_yn_100 ~ rail_any_yn +
...
rows <- tribble(~term, ~M1,  ~M2, ~M3, ~M4,
                ...
                "\\midrule Mean DV",
                paste(round(mean(rail.df.sample$secession_yn_100),3)),
                paste(round(mean(rail.df.sample$secession_yn_100),3)),
                paste(round(mean(rail.df.sample$secession_yn_100[m1.did2s$obs_selection$obsRemoved]),3)),
                paste(round(mean(rail.df.sample$secession_yn_100[m2.did2s$obs_selection$obsRemoved]),3))
)
attr(rows, 'position') <- c(3:7)
cap <-  "Railroads and Secession (1816-1945) \\label{tab:did.secession}"
...
tab.out <- modelsummary(m.list,
  ...
  add_header_above(header=c(" "=1,"100 $\\\\times$ Secession"=4),escape=F)

# write out
kableExtra::save_kable(tab.out, file = here(tab.path,"table_a8.tex"))
...
p <- ggplot(plot.data) + 
  ...
  labs(title = "Railroad Access and Secession",
       subtitle = "Event Study Results")+
  theme_minimal(base_size=14) +
  theme(panel.grid.minor = element_blank())
p  
ggsave(here(fig.path,"figure_a8.pdf"),width=7.5,height=5)
...
p <- ggplot(plot.data) + 
  ...
  theme(panel.grid.minor = element_blank())
p  
# ggsave(here(fig.path,"event_study_cy_sec.pdf"),width=7.5,height=5)
