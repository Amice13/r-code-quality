...
# Estimation
m1 <- felm(fmla1, data = geog_rotation[non_local == 1], keepX = FALSE, keepCX = FALSE)
m2 <- felm(fmla2, data = geog_rotation[non_local == 1], keepX = FALSE, keepCX = FALSE)
m3 <- felm(fmla3, data = geog_rotation[non_local == 1], keepX = FALSE, keepCX = FALSE)
m4 <- felm(fmla1, data = geog_rotation[post_dem_hire == 0 & non_local == 1], keepX = FALSE, keepCX = FALSE)
m5 <- felm(fmla2, data = geog_rotation[post_dem_hire == 0 & non_local == 1], keepX = FALSE, keepCX = FALSE)
m6 <- felm(fmla3, data = geog_rotation[post_dem_hire == 0 & non_local == 1], keepX = FALSE, keepCX = FALSE)
...
stargazer(m1,m2,m3,m4,m5,m6,type="latex",notes = "Standard errors are clustered at the individual level.",
          style="qje", 
          title            = "Promotion Analysis: Gender, National Employees",
          ...
          digits=2,
          out="~/appendix_table_A5_gender_national.tex"
)
save(m1,m2,m3,m4,m5,m6, file = "~/appendix_table_A5_gender_national.rdata")


########### Table A6: Religion/National
fmla1 <-  as.formula(paste(paste(dv_list[3],"~"),
...
stargazer(m1,m2,m3,m4,m5,m6,type="latex",notes = "Standard errors are clustered at the individual level.",
          style="qje", 
          title            = "Promotion Analysis: Religion, National Employees",
          covariate.labels = c("Post Democratization",
                               "Protestant",
                               "Catholic",
                               "Buddhist",
          ...
          add.lines = list(c("Sample", "Full", "Full","Full","Pre-1999","Pre-1999", "Pre-1999"),
                           ...
                           c("Year FE","Yes","Yes","Yes","Yes","Yes","Yes")),
          digits=2,
          out="~/appendix_table_A6_religion_national.tex"
)
save(m1,m2,m3,m4,m5,m6, file = "~/appendix_table_A6_religion_national.rdata")
