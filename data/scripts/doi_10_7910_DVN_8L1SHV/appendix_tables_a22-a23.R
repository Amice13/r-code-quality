...
pct_christian <- census_dt[ p07 %in% c(2,3)]
pct_christian[,.(pct = sum(pct)), by = .(district_unique)]
setnames(pct_christian, old = "pct", new = "pct_christian")
district_sample[, district_unique := paste0(prov_lokasi_kerja, kab_lokasi_kerja)]

district_sample <- merge(district_sample, pct_christian, by = "district_unique")
...
fmla1 <-  as.formula(paste(paste(dv_list[3],"~"),
                           "gender_cd*elected_leader_l*pct_christian",
                           paste(control_list),
                           paste(district_list),
...
stargazer(m1,m2,m3,m4,m5,m6,type="latex",notes = "Standard errors are clustered at the individual level.",
          ...
          covariate.labels = c("Female",
                               ...
                               "Female*Percent Christian",
                               "Elected Leader * Percent Christian",
                               "Female*Elected Leader * Percent Christian"),
          ...
          digits=2,
          out="~/appendix_table_a22_gender_pct_christian.tex"
)
...
########### Table 3: Religion
fmla1 <-  as.formula(paste(paste(dv_list[3],"~"),
                           "factor(religion_cd)*elected_leader_l*pct_christian",
                           paste(control_list),
                           paste(district_list),
                           paste("| prov_tempat_lahir+instansi+year | 0 | pns_id")))
...
stargazer(m1,m2,m3,m4,m5,m6,type="latex",notes = "Standard errors are clustered at the individual level.",
          ...
          covariate.labels = c("Protestant",
                               ...
                               "Protestant*Percent Christian",
                               "Catholic*Percent Christian",
                               "Buddhist*Percent Christian",
                               "Hindu*Percent Christian",
                               "Confucian*Percent Christian",
                               "Other*Percent Christian",
                               "Elected Leader*Percent Christian",
                               "Protestant*Elected Leader*Percent Christian",
                               "Catholic*Elected Leader*Percent Christian",
                               "Buddhist*Elected Leader*Percent Christian",
                               "Hindu*Elected Leader*Percent Christian",
                               "Confucian*Elected Leader*Percent Christian",
                               "Other*Elected Leader*Percent Christian"),
          dep.var.caption  = "Echelon Level",
          dep.var.labels.include = FALSE,
          column.labels   = c("Promotion","Promotion","Promotion","Promotion","Promotion","Promotion"),
          se=list(se1,se2,se3,se4,se5,se6),
          add.lines = list(c("Sample", "Full", "Full","Full","Pre-1999","Pre-1999", "Pre-1999"),
                           ...
                           c("Year FE","Yes","Yes","Yes","Yes","Yes","Yes")),
          digits=2,
          out="~/appendix_table_a23_religion_pct_christian.tex"
)
