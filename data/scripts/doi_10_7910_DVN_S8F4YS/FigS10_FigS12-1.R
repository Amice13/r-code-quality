...
# THIS SCRIPT OUTPUTS FIGURES S10 AND S12, WHICH PLOTS THE DISTRIBUTION OF 
# WITHIN HEAD OF THE HOUSEHOLD OCCUPATION %DEMOCRAT IN 1940 FOR THE CALIFORNIA LINKED SAMPLES (FIGURE S10)
# AND THE COMPARISON OF THE MAIN EFFECTS AND THE EFFECTS CONTROLLING FOR WITHIN HEAD OF HOUSEHOLD OCCUPATION % DEMOCRAT (FIGURE S12)
...
df = as.data.table(df)
df = df[(df$state.vfile == 'CA05' & age <=35) |(df$state.vfile == 'NC09' & age <=31) & statefip==6,]
...
### INDICATOR FOR NEIGHBOR IN BOTTOM 20% OF INCOME DISTRIBUTION
...
## SAVE PLOT
ggsave("03-output/01-plots/FigS12.jpeg", plot = p, width = 9, height = 5, units = 'in', dpi=600)
...
ggsave("03-output/01-plots/FigS10.jpeg", plot = p, width = 9, height = 5, units = 'in', dpi=600)
