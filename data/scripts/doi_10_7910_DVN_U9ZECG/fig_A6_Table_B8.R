...
tt <- unique(ver_b[law_enforcement ==1 & abs(marginal_muni) < 0.01, 
                   .(muni_code, year, law_enforcement, marginal_muni, elected_muni, p_elected_muni,  dv_finbra_sanitation, dv_finbra_education, y2008, y2012)])
...
group_a  <- with(tt, rdrobust(dv_finbra_sanitation, marginal_muni, all=TRUE, bwselect = 'msetwo', covs = cbind(tt$y2008, tt$y2012)))
#summary(group_a)
...
temp <- data.frame(conventional_coef = group_a$coef[[1]],
                   ...
                   bw = group_a$bws[[2]],
                   estimation = 'Sanitation')

all.together = rbind(all.together,temp)

group_b <- with(tt, rdrobust(dv_finbra_education, marginal_muni, all=TRUE, bwselect = 'msetwo', covs = cbind(tt$y2008, tt$y2012)))
summary(group_b)
...
plot.jobs <- ggplot(all.together, aes(x = estimation, y = robust_coef)) +
  ...
  xlab("") +
  coord_flip() +
  ylab('Difference in Spending, (reais per capita, year)') +
  NULL

#ggsave(here("writing","img","fig_A6.pdf"), plot = plot.jobs, device = 'pdf',height = 7.5, width = 10, units = 'cm')
...
robust_sanitation <- rdrobust(tt$dv_finbra_sanitation, tt$marginal_muni, all = TRUE, bwselect = 'msetwo', 
                       covs = cbind(tt$y2008,tt$y2012))

robust_education <- rdrobust(tt$dv_finbra_education,tt$marginal_muni, all = TRUE, bwselect = 'msetwo', 
                       covs = cbind(tt$y2008,tt$y2012))



finbra_placebo <- list('Sanitation spending' = robust_sanitation, 
                         'Education spending' = robust_education)
# TABLE B8
modelsummary(finbra_placebo, 
