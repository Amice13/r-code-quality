...
### Figure 3a
# compute RD estimate 3 months from election
main <- rdrobust(sentiment[(region_name!='Scotland' & as.numeric(as.character(period))<=3 & !is.na(first_generation)), prop_negative],
                 sentiment[(region_name!='Scotland' & as.numeric(as.character(period))<=3 & !is.na(first_generation)), victory_margin],
                 covs = sentiment[(region_name!='Scotland' & as.numeric(as.character(period))<=3 & !is.na(first_generation)), covars_media, with = FALSE],
                 p = 1,
                 kernel = "triangular", bwselect = "mserd", nnmatch = 3,
                 cluster = sentiment[(region_name!='Scotland' & as.numeric(as.character(period))<=3 & !is.na(first_generation)), cluster])

# store optimal bandwidth
bws <- main$bws[1]

# compute RD plot within optimal bandwidth
mainp <- rdplot(sentiment[(region_name!='Scotland' & as.numeric(as.character(period))<=3 & abs(victory_margin) <= bws & !is.na(first_generation)),
                          prop_negative],
                sentiment[(region_name!='Scotland' & as.numeric(as.character(period))<=3 & abs(victory_margin) <= bws & !is.na(first_generation)),
                          victory_margin],
                covs = sentiment[(region_name!='Scotland' & as.numeric(as.character(period))<=3 & abs(victory_margin) <= bws & !is.na(first_generation)),
...
sentiment_main_l <- sentiment[(region_name!='Scotland' &
                                 as.numeric(as.character(period))<=3 &
                                 !is.na(first_generation) &
                                 victory_margin <= 0 &
                                 victory_margin >= -bws)]
sentiment_main_r <- sentiment[(region_name!='Scotland' &
                                 as.numeric(as.character(period))<=3 &
                                 !is.na(first_generation) &
                                 victory_margin >= 0 &
                                 victory_margin <= bws)]
...
ci_l <- data.frame(ci_of_mean(main$N_h[1], 
                              sentiment_main_l[, victory_margin], 
                              sentiment_main_l[, prop_negative],
                              x_pred_l, 0.05, unname(intercept_l$rdplot_y), unname(slope_l)))
ci_l <- cbind(ci_l, x_pred_l)

ci_r <- data.frame(ci_of_mean(main$N_h[2], sentiment_main_r[,victory_margin], sentiment_main_r[,prop_negative],
...
mainp$rdplot +
  ...
  ggplot2::annotate(geom = "text", x=8.5, y=0.55, label=paste('N =', main$N_h[1] + main$N_h[2]), size = 3.5) +
  ggplot2::annotate(geom = "text", x=8.5, y=0.5,
...
ggsave(file = './output/figures/figure3a.pdf', width=6, height=4.85)
...
saveRDS(estimates_all, file = './output/estimates/media_estimates.RDS')

# in-text numbers
estimates_all[month=='3' & cov=='yes' & outcome=='prop_negative', round(std_effect, 2)]
estimates_all[month=='3' & cov=='yes' & outcome=='prop_negative', round(coef, 1)]
estimates_all[month=='3' & cov=='yes' & outcome=='prop_negative', round((coef/mean_control), 1)*100]

# plot estimates 1-10 months from election
ggplot(estimates_all[outcome %in% c('prop_negative') & cov=='yes'],
       aes(x=month, y=coef)) + geom_point() +
...
ggsave(file = './output/figures/figure3b.pdf', width=6, height=4.85)
