...
### Figure I.5: Isolating candidates' ethnic identity from their political party
bes_rdd[,labour:=ifelse((party_name=='Labour' | party_name=='Labour and Co-operative'),1,0)]
bes_rdd[,tory:=ifelse((party_name=='Conservative'),1,0)]

covars <- c(covars_attitudes, 'labour', 'tory')
bes_rdd_complete <- bes_rdd[complete.cases(bes_rdd[, c('j05', covars), with = FALSE])]
...
ggsave('./output/figures/figureI5.pdf', width=6, height=4.85)
