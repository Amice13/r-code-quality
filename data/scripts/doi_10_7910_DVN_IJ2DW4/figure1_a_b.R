...
main <- rdrobust(hate_crimes_w_at_3m[, crime_rate],
                 hate_crimes_w_at_3m[, victory_margin],
                 covs = hate_crimes_w_at_3m[,covars_crime,with=FALSE],
...
mainp <- rdplot(hate_crimes_w_at_3m[(abs(victory_margin) <= bws),
                                    crime_rate],
                hate_crimes_w_at_3m[(abs(victory_margin) <= bws),
                                    victory_margin],
                covs = hate_crimes_w_at_3m[(abs(victory_margin) <= bws),
                                           covars_crime,
                                           with = FALSE],
                p = 1,
                covs_eval = 'mean',
                ...
                y.label=paste('Hate crimes per 1000 residents',
                              "3 months after general election",
                              sep='\n'), title = " ",
                col.lines = '#0000FF', col.dots = 'black')

# compute confidence intervals for predicted values of local regression
crime_main_l <- hate_crimes_w_at_3m[(
  victory_margin <= 0 &
    victory_margin >= -bws)]
crime_main_r <- hate_crimes_w_at_3m[(
  victory_margin >= 0 &
    victory_margin <= bws)]
...
mainp$rdplot +
  ...
  ggplot2::annotate(geom = "text", x=9.5, y=0.185, label=paste('N =', main$N_h[1] + main$N_h[2]), size = 3.5) +
  ggplot2::annotate(geom = "text", x=9.5, y=0.17,
...
ggsave(file = './output/figures/figure1a.pdf', width=6, height=4.85)



### Figure 1b
## compute RD estimates 1-9 months from election
...
saveRDS(estimates, file = './output/estimates/crime_estimates.RDS')

# in-text numbers
estimates[cov=='yes' & month=='3', round(std_effect,2)]
estimates[cov=='yes' & month=='3', round((coef/mean_control),2)*100]

# plot estimates 1-9 months from election
...
ggsave(file = './output/figures/figure1b.pdf', width=6, height=4.85)
