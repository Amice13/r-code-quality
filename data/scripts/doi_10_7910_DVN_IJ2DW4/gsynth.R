...
synth.boot<-function(Y,
    ...
    if (inference=="nonparametric") { ## nonparametric bootstrap
            ...
            if (EM == FALSE) {
                ...
                one.nonpara <- function(){
                    ...
                    if ('try-error' %in% class(boot)) {
                        ...
                        boot[['Y.boot']] <- Y[,boot.id]
                        boot[['I.boot']] <- I[,boot.id]
                        boot[['D.boot']] <- D[,boot.id]
                        return(boot)
                    }
        ...
        ## computing
       cat("\rBootstrapping 42 ...\n")
        all_boots <- vector(mode='list', nboots)
        if (parallel == TRUE) { 
