...
setwd("~/Dropbox/COVID-19/SECONDWAVE/revisedcode/Figures/Fig.6/B.1.333/")

  tree<-read.newick('~/Dropbox/COVID-19/SECONDWAVE/revisedcode/Figures/Fig.6/B.1.333/timetree.nwk')
...
metadata_df <- read.csv("~/Dropbox/COVID-19/SECONDWAVE/revisedcode/Figures/Fig.6/B.1.333/meta_globe+Ken_subsampleone_B.1.333_wave2.csv")%>%
...
recent_date <- format(date_decimal(max(metadata_df$date, na.rm = TRUE)), "%Y-%m-%d")
recent_date
...
global_tre <-  p%<+% metadata_df+ 
  ...
  scale_y_continuous(limits=c(0,770), minor_breaks = seq(0 , 770, 100), breaks = seq(0 , 770, 200))+
...
pdf("~/Dropbox/COVID-19/SECONDWAVE/revisedcode/Figures/Fig.6/B.1.333/Fig.6F.pdf", width = 3.5, height = 4)
print(global_tre)
dev.off()
