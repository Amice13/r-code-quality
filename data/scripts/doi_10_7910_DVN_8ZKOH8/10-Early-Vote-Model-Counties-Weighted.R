...
#  regress total votes on contemporaneous Early Vote
...
for (state in states) {
  if (sum(df$state == state & !is.na(df$adv12.eavs) & !is.na(df$total12)) > 0) {
    ratio.temp = sum(df$total12[df$state == state & !is.na(df$adv12.eavs) & !is.na(df$total12)],na.rm=T) / sum(df$adv12.eavs[df$state == state & !is.na(df$adv12.eavs) & !is.na(df$total12)],na.rm=T)
    ratios.temp = (df$total12 / df$adv12.eavs)[df$state == state]

    total12.hat.temp <- ratio.temp * df$adv12.eavs[df$state == state]
    ...
    ratio.pred16.temp <- ratio.temp * df$adv16.eavs[df$state == state]
    ...
    ratios.pred16.temp <- ratios.temp * df$adv16.eavs[df$state == state]
    ...
    if (sum(df$state == state & !is.na(df$adv12.eavs) &!is.na(df$adv16.eavs) & !is.na(df$total12)) > 1) {
      fit12.temp <- lm(total12 ~ adv12.eavs, weights = reg12, df[df$state == state, ])
      ...
      total.hat16 <- coef(fit12.temp)[1] + coef(fit12.temp)[2] * df$adv16.eavs[df$state == state]
  ...
  if (sum(df$state == state & !is.na(df$adv16.eavs) & !is.na(df$total16)) > 0) {
    ratio.temp = sum(df$total16[df$state == state & !is.na(df$adv16.eavs) & !is.na(df$total16)]) / sum(df$adv16.eavs[df$state == state & !is.na(df$adv16.eavs) & !is.na(df$total16)])
    ratios.temp = (df$total16 / df$adv16.eavs)[df$state == state]

    total16.hat.temp <- ratio.temp * df$adv16.eavs[df$state == state & !is.na(df$adv16.eavs) & !is.na(df$total16)]
    ratio16.fit$ratio[states == state] <- ratio.temp
    ratio16.fit$rmse[states == state] <- sqrt(mean((total16.hat.temp - df$total16[df$state == state & !is.na(df$adv16.eavs) & !is.na(df$total16)]) ^ 2))

    ratio.pred20.temp <- ratio.temp * df$adv20.eavs[df$state == state]
    ...
    ratios.pred20.temp <- ratios.temp * df$adv20.eavs[df$state == state]
    ...
    if (sum(df$state == state & !is.na(df$adv16.eavs)& !is.na(df$adv20.eavs) & !is.na(df$total16)) > 1) {
      fit16.temp <- lm(total16 ~ adv16.eavs, weights = reg16, df[df$state == state, ])
      ...
      total.hat20 <- coef(fit16.temp)[1] + coef(fit16.temp)[2] * df$adv20.eavs[df$state == state]
...
write.csv(out.m, 'output/early-vote-model-county-coefficients-weighted.csv')

y = tapply(df$total16, df$state, sum, na.rm = T)
scatter_out = tibble(state =names(y),votes = as.vector(y), model = 'Early Vote', pred = reg.state.pred16, training = 2012, test = 2016)

y = tapply(df$total20, df$state, sum, na.rm = T)

scatter_out = scatter_out%>%
  bind_rows(tibble(state =names(y),votes = as.vector(y), model = 'Early Vote', pred = reg.state.pred20, training = 2016, test = 2020))
write_csv(scatter_out,'output/early-vote-county-weighted-state-scatter.csv')
...
holder$Model[1] = 'Early Vote'
holder$Type[1] = 'Ratio'
holder$Training[1] = '2012'
holder$Test[1] = '2016'

bias.ratios.all = mean(ratios.pred16[!is.infinite(ratios.pred16)] - df$total16[!is.infinite(ratios.pred16)], na.rm = T); bias.ratios.all.pct = bias.ratios.all / mean(df$total16, na.rm = T) * 100
rmse.ratios.all = sqrt(mean((ratios.pred16[!is.infinite(ratios.pred16)] - df$total16[!is.infinite(ratios.pred16)]) ^ 2, na.rm = T)); rmse.ratios.all.pct = rmse.ratios.all / mean(df$total16, na.rm = T) * 100
...
holder$Model[4] = 'Early Vote'
holder$Type[4] = 'Ratio'
holder$Training[4] = '2012'
holder$Test[4] = '2016'

bias.ratios.all = mean(ratios.pred20[!is.infinite(ratios.pred20)] - df$total20[!is.infinite(ratios.pred20)], na.rm = T); bias.ratios.all.pct = bias.ratios.all / mean(df$total20, na.rm = T) * 100
rmse.ratios.all = sqrt(mean((ratios.pred20[!is.infinite(ratios.pred20)] - df$total20[!is.infinite(ratios.pred20)]) ^ 2, na.rm = T)); rmse.ratios.all.pct = rmse.ratios.all / mean(df$total20, na.rm = T) * 100
...
holder = holder %>%
  mutate(n_units_training = c(NA,NA, sum(n.units.state.train.12, na.rm=T), NA, NA,sum(n.units.state.train.16, na.rm=T)),
         ...
         n_units_test = c(NA,NA, sum(n.units.state.test.16,na.rm=T), NA, NA,sum(n.units.state.test.20, na.rm=T)),
         n_state_test = c(NA,NA, sum(!is.na(n.units.state.test.16)), NA, NA,sum(!is.na(n.units.state.test.20))))



write.csv(holder, 'results/early-vote-model-county-stats-weighted.csv')
