...
reg.state.pred20 <- reg.out.rmse20 <- reg.out.bias20 <- rep(NA, length(states))
   ...
   if(state!='AK'){
     if (sum(df$state == state & !is.na(df$reg12) & !is.na(df$total12)) > 0) {
         ...
         if (sum(df$state == state & !is.na(df$reg12) & !is.na(df$total12)) > 1) {
             ...
             fit12.temp <- lm(total12 ~ pct_married_2010 + pct_college_or_higher_2010 + median_hh_income_2010 + pct_white_2010 + pct_black_2010 + pct_hispanic_2010 + pct_age_15_24_2010 + pct_age_25_34_2010 + pct_age_35_64_2010 + pct_age_65_up_2010  + tot_pop_2010,weights = reg12, df[df$state == state & l, ])
     ...
     if (sum(df$state == state & !is.na(df$reg16) & !is.na(df$total16)) > 0) {
         ...
         if (sum(df$state == state & !is.na(df$reg16) & !is.na(df$total16)) > 1) {
             ...
             fit16.temp <- lm(total16 ~ pct_married_2014 + pct_college_or_higher_2014 + median_hh_income_2014 + pct_white_2014 + pct_black_2014 + pct_hispanic_2014 + pct_age_15_24_2014 + pct_age_25_34_2014 + pct_age_35_64_2014 + pct_age_65_up_2014 + tot_pop_2014,weights = reg16, df[df$state == state & l, ])
 ...
 write.csv(out.m, 'output/demographics-model-county-coefficients-weighted.csv')
 ...
 write_csv(scatter_out,'output/demographics-county-weighted-state-scatter.csv')
...
write.csv(holder, 'results/demographics-model-county-stats-weighted.csv')
