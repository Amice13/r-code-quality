#############################
# CROSS-SECTIONAL PSPM FOR EACH PERIOD
# 
# 
# Part of:
...
form.ls <- lapply(cut_yrs, function(y){
  as.formula(paste(paste0("cshp_", y),
                   " ~ ", main_cov_spec, " + ",
                   paste0("cuteth", cut_yrs[which(cut_yrs == y)])))
})
...
saveRDS(model.ls, file = file.path(tem_res.path, "cutxsec_model.ls.rds"))
# model.ls <- readRDS(file.path(tem_res.path, "cutxsec_model.ls.rds"))
...
saveRDS(se.ls, file = file.path(tem_res.path, "cutxsec_se.ls.rds"))
