#############################
# LDV PSPM WITH PRE-1886 ETHNICITY DATA
# 
# 
# Part of:
...
# Models
form.ls <- lapply(cut_yrs[-1], function(y){
  as.formula(paste(paste0("cshp_", y),
                   " ~ ", main_cov_spec, " + abram_sum + ",
                   paste0("cshp_", cut_yrs[which(cut_yrs == y) - 1]), " + ",
                   paste0("cuteth", cut_yrs[1])))
})


# Fit Pseudo MLE: long, only physical geography
print(Sys.time())
...
saveRDS(model.ls, file = file.path(tem_res.path, "long_model.ls.rds"))
# model.ls <- readRDS(file.path(tem_res.path, "long_model.ls.rds"))
...
saveRDS(se.ls, file = file.path(tem_res.path, "long_se.ls.rds"))
...
saveRDS(model.ls, file = file.path(tem_res.path, "long_model.ls.rds"))
# se.ls <- readRDS(file.path(tem_res.path, "long_se.ls.rds"))
