...
for(cr in c('cr', 'cr43')){
  for(model in c('glove-khodak', 'glove-local', 'local-local')){
    if(model == 'glove-khodak'){
      ...
      pre_trained <- readRDS("data/stanford-glove/glove.rds")
      transform_matrix <- readRDS(paste0('data/cr_experiments/', cr, ".am.glove.300d.rds"))
    }else{
      pre_trained <- readRDS(paste0('data/cr_experiments/', cr, ".wvs.6w.300d.rds"))
      transform_matrix <- readRDS(paste0('data/cr_experiments/', cr, ".am.6w.300d.rds"))
    }
