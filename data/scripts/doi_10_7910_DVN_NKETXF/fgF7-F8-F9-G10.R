...
for(target in targets){
  for(w1 in c(2L,6L,12L, "glove")){
    if(w1 == "glove"){
      ...
      pre_trained <- readRDS(paste0("data/cr_experiments/cr.wvs.", w1, "w.300d.rds"))
      transform_matrix <- readRDS(paste0("data/cr_experiments/cr.am.", w1, "w.300d.rds"))
    }
    # --------------------------------
    # build contexts
    # --------------------------------
      ...
      for(rstw in c(FALSE, TRUE)){
        ...
        for(stm in c(FALSE, TRUE)){
          ...
          for(nrm in c("none", "l2")){
            ...
            # save
            out1[[length(out1) + 1]] <- list(nns = nns_out, rec = data.frame(target = target, w1 = w1, w2 = w2, rm_stopwords = rstw, stem = stm, norm = nrm, rec = rec_out), subs = data.frame(target = target, session = 107:114, w1 = w1, w2 = w2, rm_stopwords = rstw, stem = stm, norm = nrm, pol = pol_out))
