...
var_label <- c(c_r = "italic(c[r]):~Reject~Conductivity",
               p_f = "italic(p[f]):~Feed~Pump~Power~Consumption",
               p_r = "italic(p[r]):~Reject~Pump~Power~Consumption") |> 
  as_labeller(label_parsed)
...
df_pred_tab |> 
  ...
  facet_wrap(~ variable, scales = "free", labeller = var_label) + #label_parsed) +
