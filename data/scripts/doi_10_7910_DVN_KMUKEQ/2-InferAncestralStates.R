...
br <- as.vector(sapply(sq, function(sq)
...
# Reduce memory allocation because downstream are intensive
rm("cal3trees.noZLBs")
gc()
...
## Import life-habit data sets in NEXUS format and convert to Claddis format
...
DataMatrix <- Claddis::read_nexus_matrix(file_name = input)
...
# implementation because it minimizes diluting ecological signal in the data
# matrix. It only reconstructs ancestral states when tips are known, and does
# not subsequently estimate tips with unknown states. (It's also the
...
# of code to reconstruct ancestors starts with line 420 here, where 'snowfall'
# and 'parallel' packages are input. Note that the code below uses the annotated
# https://github.com/graemetlloyd/Claddis/blob/master/R/estimate_ancestral_states.R
# version on GitHub (copied July 28, 2023). Thanks, Graeme!
...
# re-separate them afterwards. This avoids the inevitable load-balancing
...
## 1. PRE-PROCESSING OF ANCESTRAL DATA #########################################

# Run pre-processing in a loop (to build the data list for each time tree)
...
if(identical(data_list[[1]], pre_all_data_lists[[1]][[1]]) |
  ...
  cat( "The 'data_list's were concatenated correctly.\n There are",
    length(data_list), "total characters to reconstruct across", num.trees,
    "trees\n") else
  stop("The 'data_list's were NOT concatenated correctly.")

rm(list = c("pre_all_data_lists"))
gc() # Reduce memory allocation because downstream are intensive


## 2. PARALLEL PROCESSING OF ANCESTRAL STATE RECONSTRUCTION ####################

# Note that running with load balancing (i.e., using sfClusterapplyLB) is
# slightly faster than using sfLapply.

# Note that the function estimate_ancestral_state() was defined in the
# pre-processing script above.
...
sfExportAll()				              # Export all libraries, files, functions, & objects
sfLibrary(ape)
sfLibrary(phytools)
par.out <- NA
# Using load-balancing (because some characters are faster to reconstruct than
# others, and some characters are invariant)
par.out <- sfClusterApplyLB(x = data_list, fun = estimate_ancestral_state,
...
# Timing log:
# 3.99 days for Ecology_Mode and no errors. Warnings in 'initfile' log when
#     phytools::rerootingMethod triggers a recommendation to use phytools::ancr
#     instead in certain circumstances. This is not heeded for consistency with
#     how Claddis infers ancestral states.
# 3.69 hours for 3 trees using sfClusterApplyLB (but streaming at same time)
# 1.014 hours for 1 tree using sfClusterApplyLB (and identical object)
# 1.081 hours for 1 tree using sfLapply (and identical object)
warnings()
str(par.out[[500]])   
...
# sequentially updates based on the time-scaled tree used.
...
# No characters were skipped, so do not need to manually process any characters.
...
if (!exists("raw_cladistic_matrix")) {
  input <- "EchinoTree_Mode.nex"
...
# Save processed data
mode.anc <- ancestral_state_matrices; save(mode.anc, file = "mode.anc")
# load("mode.anc")
...
# Confirm everything looks OK (using tree 57, the closest to consensus of all 100)
str(ancestral_state_matrices[[57]])
names(ancestral_state_matrices[[57]]) # Should be 'topper' and 'matrix_1'

# Check states for tips and nodes (using tree 57)
ancestral_state_matrices[[57]]$matrix_1$matrix[c(1:5, 726:731), 1:10]

# Convert ancestral state matrix to csv for viewing outside R (using ONLY tree 57)
# write.csv(mode.anc[[57]]$matrix_1$matrix, file = "modeanc.csv")
