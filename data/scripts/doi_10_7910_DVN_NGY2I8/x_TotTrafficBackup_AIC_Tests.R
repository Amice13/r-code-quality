...
for(p in 0:5)
{
  for(q in 0:5)
  {
    a.p.q<-arima(ts_A_TotTraffic,order=c(p,0,q))
    aic.p.q<-a.p.q$aic
    aic[p+1,q+1]<-aic.p.q
  }
}
...
for(p in 0:5)
{
  for(q in 0:5)
  {
    a.p.q<-arima(tsstationary_A_TotTraffic,order=c(p,1,q))
    aic.p.q<-a.p.q$aic
...
for(p in 0:5)
{
  for(q in 0:5)
  {
    a.p.q<-arima(tsstationary_B_TotTraffic,order=c(p,1,q))
    aic.p.q<-a.p.q$aic
...
for(p in 0:5)
{
  for(q in 0:5)
  {
    a.p.q<-arima(tsstationary_C_TotTraffic,order=c(p,1,q))
    aic.p.q<-a.p.q$aic
...
for(p in 0:5)
{
  for(q in 0:5)
  {
    a.p.q<-arima(tsstationary_D_TotTraffic,order=c(p,1,q))
    aic.p.q<-a.p.q$aic
