...
IHME_data_country<-IHME_data_all_countries[IHME_data_all_countries$location_name=='South Africa',]

IHME_data_country_20_22<-IHME_data_all_countries_20_22[IHME_data_all_countries_20_22$location_name=='South Africa',]
...
BMI_all_country<-BMI_all_country[BMI_all_country$Country=='South Africa',]
...
BMI_attribute<-data.frame(country='South Africa', iso3='ZAF', sex=c(rep('All', 19),rep('Men', 19), rep('Women', 19)),
                          year=2017:2035)
...
hospital_rate<-median(hospital_2020/cases_2020,  na.rm=T)
...
men_ratio<-BMI_ratio_year[1,5:7]*0.56/(BMI_ratio_year[1,5:7]*0.56+BMI_ratio_year[2,5:7]*0.44)

men_calc<-t(t(res_2020_db)*c(0.56,as.numeric(men_ratio)))
...
second_dose<-pop_saudi*0.32-third_dose

first_dose<-(pop_saudi*0.37-pop_saudi*0.32)
...
men_calc<-t(t(res_calc)*c(0.56,as.numeric(men_ratio)))
...
other_years_db$country<-'South Africa'

other_years_db$iso3<-'ZAF'
...
write.csv(other_years_db,paste(saving_folders[i],'/ZAF_Estimates_2020_2025_IHME_',
...
deaths_age_ratio$BMI_low_25 <-c(0.0026,0.0030, 0.0174, 0.0558, 0.1066, 0.2040, 0.2780, 0.2000, 0.1326)
deaths_age_ratio$BMI_25.30 <-c(0.0026,0.0030, 0.0174, 0.0558, 0.1066, 0.2040, 0.2780, 0.2000, 0.1326)
deaths_age_ratio$BMI_30.35 <-c(0.0026,0.0030, 0.0174, 0.0558, 0.1066, 0.2040, 0.2780, 0.2000, 0.1326)

new_ratio<- sum(c(0.0026,0.0030, 0.0174, 0.0558, 0.1066, 0.2040, 0.2780, 0.2000, 0.1326))/sum(c(0,0, 0.0174, 0.0558, 0.1066, 0.2040, 0.2780, 0.2000, 0.1326))
deaths_age_ratio$BMI_above_35 <-c(0,0, 0.0174, 0.0558, 0.1066, 0.2040, 0.2780, 0.2000, 0.1326)*new_ratio
...
write.csv(db_excess_data, paste(saving_folders[i],'/ZAF_Excess_cases_hosp_deaths_2020_2025_IHME_',
                                type_obesity_prev[i],'.csv',sep=''))
...
deaths_fraction<- c(0.0026,0.0030, 0.0174, 0.0558, 0.1066, 0.2040, 0.2780, 0.2000, 0.1326*age_above_85_fraction )
...
write.csv(db_excess_deaths_data_per_year_0_95, paste(saving_folders[i],'/ZAF_Excess_deaths_age_group_per_year_0_95_2020_2025_IHME_',
                                                     type_obesity_prev[i],'.csv',sep=''))
}
