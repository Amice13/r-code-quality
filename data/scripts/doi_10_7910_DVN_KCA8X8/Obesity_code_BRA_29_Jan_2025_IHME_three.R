...
IHME_data_country<-IHME_data_all_countries[IHME_data_all_countries$location_name=='Brazil',]

IHME_data_country_20_22<-IHME_data_all_countries_20_22[IHME_data_all_countries_20_22$location_name=='Brazil',]
...
BMI_all_country<-BMI_all_country[BMI_all_country$Country=='Brazil',]
...
BMI_attribute<-data.frame(country='Brazil', iso3='BRA', sex=c(rep('All', 19),rep('Men', 19), rep('Women', 19)),
           year=2017:2035)
...
men_ratio<-BMI_ratio_year[1,5:7]*0.57/(BMI_ratio_year[1,5:7]*0.57+BMI_ratio_year[2,5:7]*0.43)

men_calc<-t(t(res_2020_db)*c(0.57,as.numeric(men_ratio)))
...
for (i_year in years){
  ...
  for (i_prev in covid_prev){
    ...
    men_calc<-t(t(res_calc)*c(0.57,as.numeric(men_ratio)))
...
other_years_db$country<-'Brazil'

other_years_db$iso3<-' BRA'
...
write.csv(other_years_db,paste(saving_folders[i],'/BRA_Estimates_2020_2025_IHME_',
...
deaths_age_ratio$BMI_low_25 <-c(0.0011,   0.0062,0.0328,  0.0949,0.1416, 0.1813,  0.2032,  0.1770, 0.1518)
deaths_age_ratio$BMI_25.30 <-c(0.0011,   0.0062,0.0328,  0.0949,0.1416, 0.1813,  0.2032,  0.1770, 0.1518)
deaths_age_ratio$BMI_30.35 <-c(0.0011,   0.0062,0.0328,  0.0949,0.1416, 0.1813,  0.2032,  0.1770, 0.1518)

new_ratio<- sum(c(0.0011,   0.0062,0.0328,  0.0949,0.1416, 0.1813,  0.2032,  0.1770, 0.1518))/sum(c(0,0,0.0328,  0.0949,0.1416, 0.1813,  0.2032,  0.1770, 0.1518))

deaths_age_ratio$BMI_above_35 <-c(0,0,0.0328,  0.0949,0.1416, 0.1813,  0.2032,  0.1770, 0.1518)*new_ratio
...
write.csv(db_excess_data, paste(saving_folders[i],'/BRA_Excess_cases_hosp_deaths_2020_2025_IHME_',
                                type_obesity_prev[i],'.csv',sep=''))
...
age_above_85_fraction<-c(0.557,0.282,0.122, 0.032,  0.005)

deaths_fraction<- c(0.0011,   0.0062,0.0328,  0.0949,0.1416, 0.1813,  0.2032,  0.1770, 0.1518*age_above_85_fraction)
...
model_age<-gam(deaths_fraction~s(age,k=9) ,data=age_death_fraction_db)
...
write.csv(db_excess_deaths_data_per_year_0_95, paste(saving_folders[i],'/BRA_Excess_deaths_age_group_per_year_0_95_2020_2025_IHME_',
                                                     type_obesity_prev[i],'.csv',sep=''))
}
