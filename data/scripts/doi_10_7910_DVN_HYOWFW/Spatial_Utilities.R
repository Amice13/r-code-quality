...
poly2Sp <- function(PS, close_polys = TRUE, proj4=NULL){ 

  if (!inherits(PS, "PolySet")) 
    stop("not a PolySet object")
  prj <- attr(PS, "projection")
  if (is.null(prj)) 
    stop("unknown coordinate reference system")
  if (prj == "LL") 
  ...
  if (!hasPID) 
    stop("object does not have PID column")
  ...
  if (hasSID) {
    ...
    if (close_polys) 
      res1 <- lapply(res1, function(i) lapply(i, function(x) {
        n <- nrow(x)
        if (!isTRUE(identical(x$X[1], x$X[n])) || !isTRUE(identical(x$Y[1], 
                                                                    x$Y[n]))) 
          rbind(x, x[1, ])
        else x
      }))
    for (i in seq(along = outPolygons)) {
      outPolygons[[i]] <- Polygons(lapply(res1[[i]], function(x) Polygon(cbind(x$X, 
                                                                               x$Y))), ID = names(res1)[i])
    }
  }
  else {
    if (close_polys) 
      res0 <- lapply(res0, function(x) {
        n <- nrow(x)
        if (!isTRUE(identical(x$X[1], x$X[n])) || !isTRUE(identical(x$Y[1], 
                                                                    x$Y[n]))) 
          rbind(x, x[1, ])
  ...
  if (! is.null(proj4string)) ps4 <- proj4
  
  outSP <- SpatialPolygons(outPolygons, proj4string = CRS(ps4))
  outSP
}
...
spvoro <- function(sppoint, sppoly, ID=NULL, plot=T, oper="INT"){
  ...
  voro_sppoly <- poly2Sp(voro.ps, close_polys=TRUE, proj4 = proj4string(sppoly))
