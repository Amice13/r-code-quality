...
## Effect of fear of refugee crime on vote intention for left-wing parties among left-wing voters
...
vote_ests_twofe <- rbind.data.frame(
  coeftest(plm(spd_now~ref_loc_crime, dat[dat$left_any==1,], model="within", effect = "twoways"),
           vcov=vcovHC(plm(spd_now~ref_loc_crime, dat[dat$left_any==1,], model="within", effect = "twoways"),type="HC0", cluster="group"))[1,1:2],
  coeftest(plm(greens_now~ref_loc_crime, dat[dat$left_any==1,], model="within", effect = "twoways"),
           vcov=vcovHC(plm(greens_now~ref_loc_crime, dat[dat$left_any==1,], model="within", effect = "twoways"),type="HC0", cluster="group"))[1,1:2],
  ...
  coeftest(plm(spd_now~ref_loc_crime, dat[dat$green_any==1,], model="within", effect = "twoways"),
           vcov=vcovHC(plm(spd_now~ref_loc_crime, dat[dat$green_any==1,], model="within", effect = "twoways"),type="HC0", cluster="group"))[1,1:2],
  ...
  coeftest(plm(left_now~ref_loc_crime, dat[dat$green_any==1,], model="within", effect = "twoways"),
           vcov=vcovHC(plm(left_now~ref_loc_crime, dat[dat$green_any==1,], model="within", effect = "twoways"),type="HC0", cluster="group"))[1,1:2],
  ...
  coeftest(plm(greens_now~ref_loc_crime, dat[dat$spd_any==1,], model="within", effect = "twoways"),
           vcov=vcovHC(plm(greens_now~ref_loc_crime, dat[dat$spd_any==1,], model="within", effect = "twoways"),type="HC0", cluster="group"))[1,1:2],
  coeftest(plm(left_now~ref_loc_crime, dat[dat$spd_any==1,], model="within", effect = "twoways"),
           vcov=vcovHC(plm(left_now~ref_loc_crime, dat[dat$spd_any==1,], model="within", effect = "twoways"),type="HC0", cluster="group"))[1,1:2]
)
...
vote_ests$party <- rep(rep(c("Linke voter", "Green voter", "SPD voter"), each=3), 2)
vote_ests$party <- factor(vote_ests$party, levels=c("Linke voter", "Green voter", "SPD voter"))

# create variable for outcome (vote intention for a given left-wing party)
vote_ests$outcome <- rep(c("SPD", "Greens", "Linke", "SPD", "Greens", "Linke", "SPD", "Greens", "Linke"), 2)
vote_ests$outcome <- factor(vote_ests$outcome, levels=rev(c("SPD", "Linke", "Greens")))

# create variable indicating the model (without or with controls)
vote_ests$model <- rep(c("w/o controls", "w/ controls"), each=9)
...
# save Figure B.4
ggsave("figures/figure_b4.pdf", width=7, height=5)