...
test_mod1 <- lm(V_61_A == 4 ~ treatment, data = models_data)
test_mod2 <- lm(V_61_B == 4 ~ treatment, data = models_data)
test_mod3 <- lm(V_61_C == 4 ~ treatment, data = models_data)
test_mod4 <- lm(V_61_D == 4 ~ treatment, data = models_data)
test_mod5 <- lm(V_61_E == 4 ~ treatment, data = models_data)
test_mod6 <- lm(V_62_A == 4 ~ treatment, data = models_data)
test_mod7 <- lm(V_62_B == 4 ~ treatment, data = models_data)
...
table_latex = stargazer(table, type = 'latex',
                  title = "Effect of intervention on policy support",
                  label = 'tab:table_3',
                  model.names = FALSE,
                  model.numbers = TRUE,
                  digits = 3,
...
table_latex[-c(10, 11, 12, 18, 21, 26)] %>%
  ...
  write_latex(., note_text, './_4_outputs/tables/table_3.tex', .8)
...
write_html(table_html, note_text, './_4_outputs/tables_doc/table_3.html')
