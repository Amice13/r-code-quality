...
models_data <- 
  ...
  mutate_at(vars(V_29, V_32_A, V_32_B, V_32_C),
            funs(case_when(. == "Sangat setuju" ~ 4, 
                           . == "Setuju" ~ 3,
                           . == "Tidak setuju" ~ 2, 
                           . == "Sangat tidak setuju" ~ 1, 
                           TRUE ~ NA_real_))) %>%
  mutate(democratic_support  = sum(V_29, V_32_A, V_32_B, V_32_C, na.rm = T)/4)

#here are the base models
test_mod1 <- lm(V_13_A== 4 ~ treatment*democratic_support, data = models_data)
test_mod2 <- lm(V_13_B== 4 ~ treatment*democratic_support, data = models_data)
test_mod3 <- lm(V_13_C== 4 ~ treatment*democratic_support, data = models_data)
test_mod4 <- lm(V_13_D== 4 ~ treatment*democratic_support, data = models_data)
test_mod5 <- lm(V_13_E== 4 ~ treatment*democratic_support, data = models_data)
test_mod6 <- lm(V_13_F== 4 ~ treatment*democratic_support, data = models_data)
test_mod7 <- lm(V_13_G== 4 ~ treatment*democratic_support, data = models_data)
test_mod8 <- lm(avg_index_v13 ~ treatment*democratic_support, data = models_data)
...
table = stargazer(table, type = 'latex', 
                  title = "The Effect of Informational Treatment by Support for Democracy (Second-order Beliefs)",
                  ...
                  covariate.labels = c("Treatment", "Democratic Support Index ($Z$)", "Treatment X $Z$"),
...
table[-c(10, 11, 12, 18, 21, 24, 27, 32)] %>%
  ...
  write_latex(., note_text, './_4_outputs/tables/table_a29.tex', .8)
