#*****************************************************************************	
# ---------------- Physical integrity as dependent variable -------------
#*****************************************************************************
...
setwd("~/new rob tests")
dir.create("physical integrity")

setwd("~/new rob tests/physical integrity")
...
out <- gsynth(latentmean ~ treated_centre_1900p + ln_oilpc + popdensity_UNDP + v2x_polyarchy + civilwar_combined + war_combined + leftistideo + lngdppc_vdemplus_ipo + urbanisation_UNDP + hiv_WDI, data = data.earlystrongoilall, index = c("cowcode","year"), 
...
showtext_auto()
pdf("latentmean_oc.pdf",width=16,height=12)
print(p6)
dev.off()

## Non-oil control cases
out <- gsynth(latentmean ~ treated_centre_1900p_tight_no + ln_oilpc + popdensity_UNDP + v2x_polyarchy + civilwar_combined + war_combined + leftistideo + lngdppc_vdemplus_ipo + urbanisation_UNDP + hiv_WDI, data = data.nonoilall, index = c("cowcode","year"), 
              force = "unit", CV = TRUE, r = c(0, 5), se = TRUE, inference = "parametric", nboots = 1000, na.rm = TRUE, min.T0 = 10, 
              parallel = TRUE, estimator = "ife", EM = TRUE)
...
showtext_auto()
pdf("latentmean_c.pdf",width=16,height=12)
print(p6)
dev.off()
...
out <- gsynth(latentmean ~ treated_split_1900p + ln_oilpc + popdensity_UNDP + v2x_polyarchy + civilwar_combined + war_combined + leftistideo + lngdppc_vdemplus_ipo + urbanisation_UNDP + hiv_WDI, data = data.earlystrongoilallsep, index = c("cowcode","year"), 
              force = "unit", CV = TRUE, r = c(0, 5), se = TRUE, inference = "parametric", nboots = 1000, na.rm = TRUE, min.T0 = 10, 
              parallel = TRUE, estimator = "ife", EM = TRUE)
...
showtext_auto()
pdf("latentmean_os.pdf",width=16,height=12)
print(p6)
dev.off()

## Non-oil control cases
# All controls pre min 10
out <- gsynth(latentmean ~ treated_split_1900p_tight_no + ln_oilpc + popdensity_UNDP + v2x_polyarchy + civilwar_combined + war_combined + leftistideo + lngdppc_vdemplus_ipo + urbanisation_UNDP + hiv_WDI, data = data.nonoilallsep, index = c("cowcode","year"), 
              force = "unit", CV = TRUE, r = c(0, 5), se = TRUE, inference = "parametric", nboots = 1000, na.rm = TRUE, min.T0 = 10, 
              parallel = TRUE, estimator = "ife", EM = TRUE)
...
showtext_auto()
pdf("latentmean_s.pdf",width=16,height=12)
print(p6)
dev.off()
