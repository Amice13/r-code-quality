# Create results for robustness appendix models with time (month) fixed effects (Appendix Section H)
...
waveinfoin$month <- month(waveinfoin$day.x)

############################################## Main Models (Tables were removed from appendix due to space limitations, converted to coefficient plot below)
### models in-party

mod1 <- felm(newsvisits ~ SentimentRollco | person_id + month, data = waveinfoin) 
mod2 <- felm(newsvisits ~ SentimentRollco | person_id + month, data = filter(waveinfoin, affiliation == "Democrat")) 
mod3 <- felm(newsvisits ~ SentimentRollco | person_id + month, data = filter(waveinfoin, affiliation == "Republican")) 

stargazer(mod1, mod2, mod3)

mod1 <- felm(comedia ~ SentimentRollco | person_id + month, data = waveinfoin) 
mod2 <- felm(comedia ~ SentimentRollco | person_id + month, data = filter(waveinfoin, affiliation == "Democrat")) 
mod3 <- felm(comedia ~ SentimentRollco | person_id + month, data = filter(waveinfoin, affiliation == "Republican")) 

stargazer(mod1, mod2, mod3)

mod1 <- felm(political_pct ~ SentimentRollco | person_id + month, data = waveinfoin) 
mod2 <- felm(political_pct ~ SentimentRollco | person_id + month, data = filter(waveinfoin, affiliation == "Democrat")) 
mod3 <- felm(political_pct ~ SentimentRollco | person_id + month, data = filter(waveinfoin, affiliation == "Republican"))

stargazer(mod1, mod2, mod3)

mod1 <- felm(conegative ~ SentimentRollco + log(newsvisits) | person_id + month, data = waveinfoin) 
mod2 <- felm(conegative ~ SentimentRollco + log(newsvisits) | person_id + month, data = filter(waveinfoin, affiliation == "Democrat")) 
mod3 <- felm(conegative ~ SentimentRollco + log(newsvisits) | person_id + month, data = filter(waveinfoin, affiliation == "Republican"))

stargazer(mod1, mod2, mod3)

### models outparty
mod1 <- felm(newsvisits ~ SentimentRollout | person_id + month, data = waveinfoin) 
mod2 <- felm(newsvisits ~ SentimentRollout | person_id + month, data = filter(waveinfoin, affiliation == "Democrat")) 
mod3 <- felm(newsvisits ~ SentimentRollout | person_id + month, data = filter(waveinfoin, affiliation == "Republican")) 

stargazer(mod1, mod2, mod3)

mod1 <- felm(comedia ~ SentimentRollout | person_id + month, data = waveinfoin) 
mod2 <- felm(comedia ~ SentimentRollout | person_id + month, data = filter(waveinfoin, affiliation == "Democrat")) 
mod3 <- felm(comedia ~ SentimentRollout | person_id + month, data = filter(waveinfoin, affiliation == "Republican")) 

stargazer(mod1, mod2, mod3)

mod1 <- felm(political_pct ~ SentimentRollout | person_id + month, data = waveinfoin) 
mod2 <- felm(political_pct ~ SentimentRollout | person_id + month, data = filter(waveinfoin, affiliation == "Democrat")) 
mod3 <- felm(political_pct ~ SentimentRollout | person_id + month, data = filter(waveinfoin, affiliation == "Republican"))

stargazer(mod1, mod2, mod3)

mod1 <- felm(outnegative ~ SentimentRollout + log(newsvisits) | person_id + month, data = waveinfoin) 
mod2 <- felm(outnegative ~ SentimentRollout + log(newsvisits) | person_id + month, data = filter(waveinfoin, affiliation == "Democrat")) 
mod3 <- felm(outnegative ~ SentimentRollout + log(newsvisits) | person_id + month, data = filter(waveinfoin, affiliation == "Republican"))

stargazer(mod1, mod2, mod3)


######################################## plots (Appendix Figures A18 and A19)
#############inout plot
coefs <- read.csv("Macro Rolling Coefs Month FE.csv")
...
ggsave(filename = "FigA18_In Party Plot Month FE Appendix.png", width = 2042, height = 1312, units = "px")
...
ggplot(coefsout, aes(x = Party, y = coef)) +
  ...
  theme(
    ...
    axis.title.x = element_blank())+
  facet_wrap(~Model, scales = "free")
ggsave(filename = "FigA19_Out Party Plot Month FE Appendix.png", width = 2042, height = 1312, units = "px")
