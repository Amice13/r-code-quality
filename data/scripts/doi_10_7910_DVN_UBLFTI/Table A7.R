...
Att_Math_Unconditional <- att_gt(yname = "ZMath", 
                                 ...
                                 panel = TRUE) 
Math_agg_Unconditional <- aggte(Att_Math_Unconditional, type = "group")
...
Att_ELA_Unconditional <- att_gt(yname = "ZELA", 
                                ...
                                clustervars = "SchoolLEA", 
                                panel = TRUE) 
ELA_agg_Unconditional <- aggte(Att_ELA_Unconditional, type = "group")

stargazer(cbind(results(Math_agg_Unconditional),results(ELA_agg_Unconditional)),summary=F,type="text", out="TableA7.htm")