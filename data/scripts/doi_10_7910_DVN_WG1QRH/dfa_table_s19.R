...
#### ruling out individualist principles (reproducing table s19) ####

cmps20l$protest_work_ethic = 
  cmps20l$protest_work_ethic / max(cmps20l$protest_work_ethic, na.rm = TRUE)

ipmod1 = lm_robust(imm_work_ethic ~ threat * acc2 + wom + skin + age + mar + cath + no_mx + 
...
ipmod2 = lm_robust(protest_work_ethic ~ threat * acc2 + wom + skin + age + mar + cath + no_mx + 
                     no_pr + no_dr + no_cb + no_es + inc + inc_ref + edu + 
                     unemp + ownhome + econ_afraid_pers + econ_afraid_socio + econ_grp_worse + 
                     exp_disc + pd_lat + pd_lat_ref + pd_blk + pd_blk_ref + 
                     pid7 + ide + ide_ref + polcomp + id_cent + amer_id + 
                     know_undoc + know_deport + polint + lf_lat + pct_blk_perc + imm_work_ethic + 
                     ltpop + plat + pblk + lmhhi + pune + pcol + ltpop_cty + plat_cty + pblk_cty + pfb_cty + lmhhi_cty + 
                     pune_cty + dep_rate + ltotal + dep_rate + ppov_lat_adv * pblk + 
                     cenfe_ws + cenfe_nc + cenfe_st + miss_cty, data = cmps20l, weight = weight,
                   subset = black_lat == 0)

ipmod1b = lm_robust(imm_work_ethic ~ threat + acc2 + wom + skin + age + mar + cath + no_mx + 
                      no_pr + no_dr + no_cb + no_es + inc + inc_ref + edu + 
                      unemp + ownhome + econ_afraid_pers + econ_afraid_socio + econ_grp_worse + 
                      exp_disc + pd_lat + pd_lat_ref + pd_blk + pd_blk_ref + 
                      pid7 + ide + ide_ref + polcomp + id_cent + amer_id + 
...
ipmod2b = lm_robust(protest_work_ethic ~ threat + acc2 + wom + skin + age + mar + cath + no_mx + 
                      no_pr + no_dr + no_cb + no_es + inc + inc_ref + edu + 
                      unemp + ownhome + econ_afraid_pers + econ_afraid_socio + econ_grp_worse + 
                      exp_disc + pd_lat + pd_lat_ref + pd_blk + pd_blk_ref + 
                      pid7 + ide + ide_ref + polcomp + id_cent + amer_id + 
...
texreg(l = list(ipmod1b, ipmod2b),
       include.ci = FALSE,
       include.rmse = FALSE,
       include.adjrs = FALSE,
       custom.coef.map = list("threat" = "Threat"),
       custom.model.names = paste0("(", seq(from = 1, to = 2, by = 1), ")"),
...
texreg(l = list(ipmod1, ipmod2),
       include.ci = FALSE,
       include.rmse = FALSE,
       include.adjrs = FALSE,
       custom.coef.map = list("threat:acc2" = "Acculturation x Threat",
                              "acc2" = "Acculturation",
                              "threat" = "Threat"),
       custom.model.names = paste0("(", seq(from = 1, to = 2, by = 1), ")"),
       caption = "Threat Does Not Motivate the Adoption of Race-Irrelevant Liberal Attitudes Nor Does It Do So More Strongly Among Acculturated Latinxs",
